...
BOOL CQuery::SQLSyntexCheck(char* SQLString)
{
	char* temp;
...
BOOL CQuery::SpaceSyntexCheck(char* string)
{
	char* temp;
...
int CQuery::ReadBlob(LPCTSTR szSQL, void *buf)
{
	...
	{
		p = (char *)buf;
		while ((ret = SQLGetData(m_hStmt, 1, SQL_C_BINARY, BinaryPtr, sizeof(BinaryPtr),&LenBin)) != SQL_NO_DATA) 
		{
