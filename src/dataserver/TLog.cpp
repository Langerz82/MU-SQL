...
void TLog::LogDataSet_Start()
{
	...
	wsprintf(szTemp, "Log\\%02d.%02d.%04d_%s.log", this->m_StartDate.Day, this->m_StartDate.Month, this->m_StartDate.Year, "DataServer");
	...
	if (IsFile(szTemp) == TRUE)
	{
		for (int i = 2; ; i++)
		{
			...
			wsprintf(szTemp, "Log\\%02d.%02d.%04d_%s_start-%d.log", this->m_StartDate.Day, this->m_StartDate.Month, this->m_StartDate.Year, "DataServer", i);
...
void TLog::LogDataSet_Check()
{
	...
	else
	{
		...
		CreateDirectory("Log", NULL);
		wsprintf(szTemp, "Log\\%02d.%02d.%04d_-_%02d.%02d.%04d_%s.log", this->m_StartDate.Day, this->m_StartDate.Month, this->m_StartDate.Year, this->m_CurDate.Day, this->m_CurDate.Month, this->m_CurDate.Year, "DataServer");

		char szTemp2[255];
		sprintf(szTemp2, "Log\\%02d.%02d.%04d_%s.log", this->m_CurDate.Day, this->m_CurDate.Month, this->m_CurDate.Year, "DataServer");
...
void TLog::AddMSB(char * szLog, ...)
{
	...
	MessageBox(NULL, szLogBuff, "[WARNING]", MB_OK|MB_TOPMOST);
}
...
void TLog::Add(char * szLog, ...)
{
	...
	today = localtime(&ltime);
		...
		try 
			...
			{ 
				sprintf(szLogBuff, "[%.8s] [ANTI_CRASH_HACK] [TLog::AddMSB] ", asctime(today) + 11); strcat(szLogBuff, szLog); 
...
void TLog::AddC(TColor::T tColor, char * szLog, ...)
{
	...
	today = localtime(&ltime);
		...
		try 
			...
			{ 
				sprintf(szLogBuff, "[%.8s] [ANTI_CRASH_HACK] [TLog::AddC] ", asctime(today) + 11); strcat(szLogBuff, szLog); 
				LogTextAdd(TColor::Red, szLogBuff, strlen(szLogBuff)); 
				return; 
			} 
