////////////////////////////////////////////////////////////////////////////////
// NewPVP.cpp
#include "StdAfx.h"
#include "NewPVP.h"
#include "Main.h"
#include "TNotice.h"
#include "util.h"
#include "BuffEffect.h"
#include "BuffEffectSlot.h"
#include "MoveCommand.h"
#include "DarkSpirit.h"
#include "Gate.h"
#include "Logging/Log.h"
#include "PartyClass.h"
#include "BagManager.h"
#include "MineSystem.h"
#include "MuunSystem.h"
#include "IniReader.h"
#include "GameProtocol.h"
#include "GOFunctions.h"

extern PartyClass gParty;

CNewPVP g_NewPVP;

//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CNewPVP::CNewPVP(): m_bNewPVP(FALSE)
{
	memset(&m_DuelChannelList, 0, sizeof(m_DuelChannelList));
	memset(m_DuelChannel, 0, sizeof(m_DuelChannel));
	memset(&m_ObserverCount, 0, sizeof(m_ObserverCount));

	m_DuelChannelList.h.c = 0xC1;
	m_DuelChannelList.h.headcode = 0xAA;
	m_DuelChannelList.h.subcode = 0x06;
	m_DuelChannelList.h.size = sizeof(PMSG_ANS_DUEL_CHANNELLIST);

	//InitializeCriticalSection(&this->m_csWaiting);
	//InitializeCriticalSection(&this->m_csObserver);
}

CNewPVP::~CNewPVP()
{
	m_ObserverInfoList.clear();
	m_Waiting.clear();
	//DeleteCriticalSection(&this->m_csWaiting);
	//DeleteCriticalSection(&this->m_csObserver);
}

void CNewPVP::LoadData()
{
	CIniReader ReadDuel(g_ConfigRead.GetPath("IGC_Common.ini"));

	m_bNewPVP = ReadDuel.ReadInt("DuelSystem", "Enable", FALSE);
	m_iDuelDamageReduction = ReadDuel.ReadInt("DuelSystem", "DamageReduction", 60);

	sLog->outBasic("[NewPVP] Info Set Enable:%d DamageReduction:%d", m_bNewPVP, m_iDuelDamageReduction);
}

static int g_GateObserver[DUEL_CHANNEL_MAX] =  {	303,304,305,306 };
static int g_GateRequester[DUEL_CHANNEL_MAX] = {	295,297,299,301 };
static int g_GateResponsor[DUEL_CHANNEL_MAX] = {	296,298,300,302 };

void CNewPVP::Run()
{
	////EnterCriticalSection(&this->m_csWaiting);
	std::map<int,Waiting>::iterator iter = m_Waiting.begin();

	while(iter!=m_Waiting.end())
	{
		Waiting & waiting = iter->second;
		if(waiting.bExpired == 1)
		{
			iter = m_Waiting.erase(iter);
			continue;
		}

		DWORD dwRemainMsec = GetTickCount() - waiting.dwTime;			
		if( dwRemainMsec > 10000 )
		{
			CGameObject &requester = *getGameObject(waiting.nRequester);
			CGameObject &responsor = *getGameObject(waiting.nResponsor);
			Cancel(requester, responsor, FALSE);
		}		
		iter++;
	}

	////LeaveCriticalSection(&this->m_csWaiting);
	
	for(int i = 0;i < DUEL_CHANNEL_MAX;i++)
	{
		int nStatus = m_DuelChannel[i].nStatus;
		switch(nStatus)
		{
		case 3:
			if(!CheckUsersOnConnect(i))	continue;
			break;
		case 2:
			{
				DWORD dwRemainMsec = GetTickCount() - m_DuelChannel[i].dwTime;
				int nNotifySec = 6 - dwRemainMsec / 1000;				
				if(nNotifySec < 6 && m_DuelChannel[i].nNotifyCount != nNotifySec )
				{
					m_DuelChannel[i].nNotifyCount = nNotifySec;
					BroadcastMessage(i, 3, 0, nNotifySec);
				}
				if(dwRemainMsec > 6000)
				{	
					if(!CheckUsersOnConnect(i))	continue;
					
					CGameObject &requester = *getGameObject(m_DuelChannel[i].nIndex1);
					CGameObject &responsor = *getGameObject(m_DuelChannel[i].nIndex2);
					
					gObjClearBuffEffect(requester, CLEAR_TYPE_NON_PCS_ITEM_EFFECT);
					gObjClearBuffEffect(responsor, CLEAR_TYPE_NON_PCS_ITEM_EFFECT);

					ReFill(requester);
					ReFill(responsor);

					BroadcastRound(i, 2, FALSE);
					SetStatus(3, i, requester, responsor);
				}
				break;
			}
		case 4:
			{
				DWORD dwRemainMsec = GetTickCount() - m_DuelChannel[i].dwTime;
				if(dwRemainMsec > 5000)
				{
					if(!CheckUsersOnConnect(i))	continue;
					
					CGameObject &requester = *getGameObject(m_DuelChannel[i].nIndex1);
					CGameObject &responsor = *getGameObject(m_DuelChannel[i].nIndex2);

					MoveGate(requester, g_GateRequester[i]);
					MoveGate(responsor, g_GateResponsor[i]);
					ReFill(requester);
					ReFill(responsor);
					BroadcastRound( i, 2, FALSE);
					SetStatus(3, i, requester, responsor);
				}
			}
			break;
		case DC_RESERVEDEND:
			{
				DWORD dwRemainMsec  = GetTickCount() - m_DuelChannel[i].dwTime;
				int nNotifySec = 10 - dwRemainMsec / 1000;
				if( nNotifySec < 6 && m_DuelChannel[i].nNotifyCount != nNotifySec )
				{
					m_DuelChannel[i].nNotifyCount = nNotifySec;
					BroadcastMessage(i, 3, 2, nNotifySec);
				}
				if( dwRemainMsec > 10000 )
				{
					CGameObject &requester = *getGameObject(m_DuelChannel[i].nIndex1);
					CGameObject &responsor = *getGameObject(m_DuelChannel[i].nIndex2);
					Leave( requester, responsor);
					LeaveChannelObserver(i);
					SetStatus(DC_IDLE, i, requester, responsor);
				}
			}
			break;
		}
		if( IS_START(nStatus) )	BroadcastDuelInfo(i, 2);
	}
}

int CNewPVP::GetDuelChannelId()
{
	for(int i = 0;i < DUEL_CHANNEL_MAX;i++)
	{
		if( m_DuelChannel[i].nStatus == DC_IDLE)
		{
			return i;
		}
	}

	return -1;
}

int CNewPVP::GetDuelChannelId(int nIndex)
{
	for( int i = 0; i < DUEL_CHANNEL_MAX; i++ )
	{
		if( m_DuelChannel[i].nIndex1 == nIndex || 
			m_DuelChannel[i].nIndex2 == nIndex )
		{
			return i;
		}
	}

	return -1;
}

int CNewPVP::GetDuelChannelId(char* lpszName)
{
	for( int i = 0; i < DUEL_CHANNEL_MAX; i++ )
	{
		if( strcmp(m_DuelChannel[i].szName1, lpszName) == 0|| 
			strcmp(m_DuelChannel[i].szName2, lpszName) == 0)
		{
			return i;
		}
	}
	
	return -1;
}

LPDUEL_CHANNEL CNewPVP::GetDuelChannel()
{
	for( int i = 0; i < DUEL_CHANNEL_MAX; i++ )
	{
		if( m_DuelChannel[i].nStatus == DC_IDLE )
		{
			return &m_DuelChannel[i];
		}
	}

	return NULL;
}

LPDUEL_CHANNEL CNewPVP::GetDuelChannel(char* lpszName)
{
	for( int i = 0; i < DUEL_CHANNEL_MAX; i++ )
	{
		if( strcmp(m_DuelChannel[i].szName1, lpszName) == 0 || 
			strcmp(m_DuelChannel[i].szName2, lpszName) == 0)
		{
			return &m_DuelChannel[i];
		}
	}

	return 0;
}

void CNewPVP::SetStatus(int nStatus, int nId, CGameObject &requester, CGameObject &responsor)
{
	LPDUEL_CHANNEL lpDuelChannel = &m_DuelChannel[nId];

	switch ( nStatus )
	{		
	case DC_IDLE:
		lpDuelChannel->nStatus = nStatus;
		lpDuelChannel->dwTime = 0;

		lpDuelChannel->nIndex1 = 0;
		lpDuelChannel->nIndex2 = 0;

		memset(lpDuelChannel->szName1, 0, MAX_ACCOUNT_LEN);
		memset(lpDuelChannel->szName2, 0, MAX_ACCOUNT_LEN);

		m_DuelChannelList.channel[nId].bStart = 0;
		m_DuelChannelList.channel[nId].bWatch = 0;

		memset(m_DuelChannelList.channel[nId].szName1, 0, MAX_ACCOUNT_LEN);
		memset(m_DuelChannelList.channel[nId].szName2, 0, MAX_ACCOUNT_LEN);

		SetDuelStatus(requester, responsor, nStatus);
		break;
	case 2:
		lpDuelChannel->nStatus = nStatus;
		lpDuelChannel->dwTime = GetTickCount();

		lpDuelChannel->nIndex1 = requester.m_Index;
		lpDuelChannel->nIndex2 = responsor.m_Index;

		std::memcpy(lpDuelChannel->szName1, requester.Name, MAX_ACCOUNT_LEN);
		std::memcpy(lpDuelChannel->szName2, responsor.Name, MAX_ACCOUNT_LEN);

		m_DuelChannelList.channel[nId].bStart = 1;
		m_DuelChannelList.channel[nId].bWatch = 1;

		std::memcpy(m_DuelChannelList.channel[nId].szName1, requester.Name, MAX_ACCOUNT_LEN);
		std::memcpy(m_DuelChannelList.channel[nId].szName2, responsor.Name, MAX_ACCOUNT_LEN);

		SetDuelStatus(requester, responsor, nStatus);
	  break;
	case 3:
	  lpDuelChannel->nStatus = nStatus;
	  lpDuelChannel->dwTime = GetTickCount();
	  break;
	case 4:
	  lpDuelChannel->nStatus = nStatus;
	  lpDuelChannel->dwTime = GetTickCount();
	  break;
	case DC_RESERVEDEND:
	  lpDuelChannel->nStatus = nStatus;
	  lpDuelChannel->dwTime = GetTickCount();
	  break;
	}
}

void CNewPVP::SetDuelStatus(CGameObject &requester, CGameObject &responsor, int nStatus)
{
	switch(nStatus)
	{
	case DC_IDLE:
		requester.m_iDuelUser = -1;
		requester.m_iDuelUserRequested = -1;
		requester.m_iDuelUserReserved = -1;
		
		responsor.m_iDuelUser = -1;
		responsor.m_iDuelUserRequested = -1;
		responsor.m_iDuelUserReserved = -1;
		break;
	case 1:
		requester.m_iDuelUserReserved = responsor.m_Index;
		requester.m_iDuelUser = -1;
		
		responsor.m_iDuelUserRequested = requester.m_Index;
		break;
	case 2:
		requester.m_iDuelUserReserved = -1;
        requester.m_btDuelScore = 0;
        requester.m_iDuelUser = responsor.m_Index;
        requester.m_iDuelTickCount = GetTickCount();
        
		responsor.m_iDuelUserRequested = -1;
        responsor.m_iDuelUserReserved = -1;
        responsor.m_btDuelScore = 0;
        responsor.m_iDuelUser = requester.m_Index;
        responsor.m_iDuelTickCount = GetTickCount();
		break;
	}
}

int CNewPVP::Reserve(CGameObject &requester,CGameObject &responsor)
{
	int nRet = GetDuelStatus(requester);
	if(nRet)	return nRet;

	nRet = GetDuelStatusByResponsor(requester);
	if(nRet)	return nRet;

	LPMOVE_COMMAND_DATA lpData = gMoveCommand.GetMoveCommandData(42);
	if(lpData == NULL){ 
		sLog->outBasic("%s\t%s\t%s\t%s\t%d","lpData","ENEWPVP::E_NOT_FOUND_MOVEREQUESTDATA","NULL",__FILE__, __LINE__);
		return ENEWPVP::E_NOT_FOUND_MOVEREQUESTDATA; }

	if( !IsVulcanusMap(requester.MapNumber) )
	{
		if(IsVulcanusMap(requester.MapNumber) || requester.m_PlayerData->Money - lpData->NeedZen < 0)	return ENEWPVP::E_NOT_ENOUGH_MONEY;
	}
	if( !IsVulcanusMap(responsor.MapNumber) )
	{
		if(IsVulcanusMap(responsor.MapNumber) || responsor.m_PlayerData->Money - lpData->NeedZen < 0)	return ENEWPVP::E_NOT_ENOUGH_MONEY;
	}

	if( !CheckLimitLevel(requester,g_GateRequester[0]) )	return ENEWPVP::E_LIMIT_LEVEL;
	if( !CheckLimitLevel(responsor,g_GateResponsor[0]) )	return ENEWPVP::E_LIMIT_LEVEL;

	Waiting waiting = {0};
    waiting.nRequester = requester.m_Index;
    waiting.nResponsor = responsor.m_Index;
    waiting.bExpired = 0;
    waiting.dwTime = GetTickCount();

	////EnterCriticalSection(&this->m_csWaiting);

	std::pair< std::map<int,Waiting>::iterator, bool > pair = m_Waiting.insert( std::make_pair(requester.m_Index,  waiting) );
	if(pair.second == false)
	{
		sLog->outBasic("%s\t%s\t%s\t%s\t%d","pair.second","ENEWPVP::E_FAILED_ENTER","NULL",__FILE__,__LINE__);

		////LeaveCriticalSection(&this->m_csWaiting);
		return ENEWPVP::E_FAILED_ENTER;
	}

	////LeaveCriticalSection(&this->m_csWaiting);

	SetDuelStatus(requester, responsor, 1);

	PMSG_REQ_DUEL_ANSWER req;
    req.h.c = 0xC1;
    req.h.size = sizeof(PMSG_REQ_DUEL_ANSWER);
    req.h.headcode = 0xAA;
    req.h.subcode = 0x02;
    req.NumberH = HIBYTE(requester.m_Index);
    req.NumberL = LOBYTE(requester.m_Index);
    std::memcpy(req.szName, requester.Name, MAX_ACCOUNT_LEN);
    GIOCP.DataSend(responsor.m_Index, (BYTE*)&req, req.h.size);
	return 0;
}

int CNewPVP::Join(CGameObject &requester, CGameObject &responsor)
{
	if( IsDuel(requester) ) return ENEWPVP::E_ALREADY_DUELLING_1;
	if( IsDuelReserved(responsor) ) return ENEWPVP::E_ALREADY_DUELRESERVED_1;
	if( requester.m_iDuelUserReserved != responsor.m_Index ) return ENEWPVP::E_INVITE_MYSELF;

	////EnterCriticalSection(&this->m_csWaiting);

	std::map<int,Waiting>::iterator iter = m_Waiting.find(requester.m_Index);

	if(!(iter!=m_Waiting.end()))
	{ 
		sLog->outBasic("%s\t%s\t%s\t%s\t%d","iter!=m_Waiting.end()","ENEWPVP::E_NOT_EXIST_USER","NULL", __FILE__,  __LINE__);

		////LeaveCriticalSection(&this->m_csWaiting);
		return ENEWPVP::E_NOT_EXIST_USER;
	}

	Waiting & waiting = iter->second;
	waiting.bExpired = 1;
	////LeaveCriticalSection(&this->m_csWaiting);

	int	nId = GetDuelChannelId();
	if( nId < 0 || nId >= DUEL_CHANNEL_MAX ) return ENEWPVP::E_DUEL_MAX;

	if( m_DuelChannel[nId].nStatus != DC_IDLE ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","m_DuelChannel[nId].nStatus==DC_IDLE","ENEWPVP::E_INVALID_STATUS","NULL", __FILE__, __LINE__); return ENEWPVP::E_INVALID_STATUS; }

	LPMOVE_COMMAND_DATA lpData = gMoveCommand.GetMoveCommandData(42);	
	if( lpData == NULL ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","lpData","ENEWPVP::E_NOT_FOUND_MOVEREQUESTDATA","NULL",__FILE__, __LINE__); return ENEWPVP::E_NOT_FOUND_MOVEREQUESTDATA; }

	if( !CheckLimitLevel(requester, g_GateRequester[nId]) )	return ENEWPVP::E_LIMIT_LEVEL;
	if( !CheckLimitLevel(responsor, g_GateResponsor[nId]) ) return ENEWPVP::E_LIMIT_LEVEL;

	if( !IsVulcanusMap(requester.MapNumber) )
	{
		if( requester.m_PlayerData->Money - lpData->NeedZen >= 0 )
		{
			requester.m_PlayerData->Money -= lpData->NeedZen;
			GSProtocol.GCMoneySend(&requester, requester.m_PlayerData->Money);
		}
		else return ENEWPVP::E_NOT_ENOUGH_MONEY;
   	}
	
	if( !IsVulcanusMap(responsor.MapNumber) )
	{
		if( responsor.m_PlayerData->Money - lpData->NeedZen >= 0 )
		{
			responsor.m_PlayerData->Money -= lpData->NeedZen;
			GSProtocol.GCMoneySend(&responsor, responsor.m_PlayerData->Money);
		}
		else return ENEWPVP::E_NOT_ENOUGH_MONEY;
	}

	if (requester.m_PlayerData->m_bIsMining)
		g_MineSystem.CheckMoveMapWhileMining(requester);
	if (responsor.m_PlayerData->m_bIsMining)
		g_MineSystem.CheckMoveMapWhileMining(responsor);

	SetStatus(2, nId, requester, responsor);

	PMSG_ANS_DUEL_INVITE res = {0};
	PMSG_NOTICE pNotice;
	res.h.c = 0xC1;
	res.h.size = sizeof(PMSG_ANS_DUEL_INVITE);
	res.h.headcode = 0xAA;
	res.h.subcode = 0x01;
	res.nResult = 0x00;
	std::memcpy(res.szName, responsor.Name, MAX_ACCOUNT_LEN);

	res.NumberH = HIBYTE(requester.m_Index);
	res.NumberL = LOBYTE(requester.m_Index);
	std::memcpy(res.szName, requester.Name, MAX_ACCOUNT_LEN);
	GIOCP.DataSend(responsor.m_Index, (BYTE*)&res,res.h.size);

	res.NumberH = HIBYTE(responsor.m_Index);
	res.NumberL = LOBYTE(responsor.m_Index);
	std::memcpy(res.szName, responsor.Name, MAX_ACCOUNT_LEN);
	GIOCP.DataSend(requester.m_Index, (BYTE*)&res, res.h.size);
	
	BroadcastScore(nId, 1);
	MoveGate(requester, g_GateRequester[nId]);
	MoveGate(responsor, g_GateResponsor[nId]);

	TNotice::MakeNoticeMsgEx(&pNotice, 0, Lang.GetText(0,309), responsor.Name, requester.Name, nId+1);
	TNotice::SendNoticeToAllUser(&pNotice);

	sLog->outBasic("[NewPVP] [%s][%s] Duel Started [%s][%s]",responsor.m_PlayerData->ConnectUser->AccountID,responsor.Name,
		requester.m_PlayerData->ConnectUser->AccountID, requester.Name);
	return 0;
}

void CNewPVP::Cancel(CGameObject &requester, CGameObject &responsor, BOOL bSendPacket)
{
	////EnterCriticalSection(&this->m_csWaiting);
	std::map<int,Waiting>::iterator iter = m_Waiting.find(requester.m_Index); 

	if(!(iter!=m_Waiting.end()))
	{ 
		sLog->outBasic("%s\t%s\t%s\t%s\t%d","iter!=m_Waiting.end()","0","NULL", __FILE__,  __LINE__);
		////LeaveCriticalSection(&this->m_csWaiting);
		return; 
	}

	Waiting & waiting = iter->second;
	waiting.bExpired = 1;
	////LeaveCriticalSection(&this->m_csWaiting);

	SetDuelStatus(requester, responsor, DC_IDLE);

	if( bSendPacket == TRUE )
    {
		PMSG_ANS_DUEL_INVITE res = {0};
		res.h.c = 0xC1;
		res.h.size = sizeof(PMSG_ANS_DUEL_INVITE);
		res.h.headcode = 0xAA;
		res.h.subcode = 0x01;
		std::memcpy(res.szName, responsor.Name, MAX_ACCOUNT_LEN);
		res.nResult = 0x0F;
      
		if( gObjIsConnected(requester) )	GIOCP.DataSend(requester.m_Index, (BYTE*)&res,res.h.size);
		if( gObjIsConnected(responsor) )	GIOCP.DataSend(responsor.m_Index, (BYTE*)&res, res.h.size);
    }
}

int CNewPVP::Leave(CGameObject &Obj)
{
	int nId = GetDuelChannelId(Obj.m_Index);
	if(nId < 0 || nId >= DUEL_CHANNEL_MAX){	sLog->outBasic("%s\t%s\t%s\t%s\t%d","nId>=0 && nId<DUEL_CHANNEL_MAX","ENEWPVP::E_INVALID_CHANNELID","NULL",__FILE__, __LINE__); return ENEWPVP::E_INVALID_CHANNELID; }
	if( !IS_START(m_DuelChannel[nId].nStatus ) && m_DuelChannel[nId].nStatus != DC_RESERVEDEND ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","IS_START(m_DuelChannel[nId].nStatus)||m_DuelChannel[nId].nStatus==DC_RESERVEDEND", "ENEWPVP::E_INVALID_STATUS","NULL",__FILE__, __LINE__); return ENEWPVP::E_INVALID_STATUS; }	

	CGameObject* lpTargetObj = NULL;

	Obj.m_iDuelUserReserved = -1;

	if( IsDuel(Obj) )	lpTargetObj = (CGameObject*)getGameObject(Obj.m_iDuelUser);

	if(lpTargetObj == NULL){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","lpTargetObj","ENEWPVP::E_INVALID_INDEX","NULL", __FILE__, __LINE__); return ENEWPVP::E_INVALID_INDEX; }

	if( m_DuelChannel[nId].nStatus != DC_RESERVEDEND )
    {
        SetStatus(DC_RESERVEDEND, nId, Obj, *lpTargetObj);
        BroadcastMessage( nId, 1, 1, 10);
    }
	return 0;
}

void CNewPVP::Leave(CGameObject &requester, CGameObject &responsor)
{
	int nId = GetDuelChannelId(requester.m_Index);
	if(nId < 0 || nId >= DUEL_CHANNEL_MAX){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","nId>=0 && nId<DUEL_CHANNEL_MAX","0","NULL",__FILE__, __LINE__); return; }
	if( !IS_START(m_DuelChannel[nId].nStatus) && m_DuelChannel[nId].nStatus != DC_RESERVEDEND ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","IS_START(m_DuelChannel[nId].nStatus)||m_DuelChannel[nId].nStatus==DC_RESERVEDEND", "0","NULL",__FILE__, __LINE__); return; }	

	PMSG_ANS_DUEL_EXIT res = {0};
	res.h.c = 0xC1;
    res.h.headcode = 0xAA;
    res.h.subcode = 0x03;
    res.h.size = sizeof(PMSG_ANS_DUEL_EXIT);
    res.nResult = 0x00;

	if( gObjIsConnected(requester) )
    {
		requester.m_iDuelUser = -1;
        res.NumberH = HIBYTE(responsor.m_Index);
        res.NumberL = LOBYTE(responsor.m_Index);
        std::memcpy(res.szName, responsor.Name, MAX_ACCOUNT_LEN);

        GIOCP.DataSend(requester.m_Index, (BYTE*)&res, res.h.size);
        
		if( IsPKFieldMap(requester.MapNumber) )
		{
			gObjMoveGate(requester, 17);
		}
    }
	
	if( gObjIsConnected(responsor) )
	{
        responsor.m_iDuelUser = -1;
        res.NumberH = HIBYTE(responsor.m_Index);
        res.NumberL = LOBYTE(responsor.m_Index);
        std::memcpy(res.szName, requester.Name, MAX_ACCOUNT_LEN);
        
		GIOCP.DataSend(responsor.m_Index, (BYTE*)&res, res.h.size);
        
		if( IsPKFieldMap(responsor.MapNumber) )
		{
			gObjMoveGate(responsor, 17);
		}
    }
}

void CNewPVP::SetScore(CGameObject &Obj)
{
	int nId = GetDuelChannelId(Obj.m_Index);
	if(nId < 0 || nId >= DUEL_CHANNEL_MAX) { sLog->outBasic("%s\t%s\t%s\t%s\t%d","nId>=0 && nId<DUEL_CHANNEL_MAX","0","NULL",__FILE__, __LINE__); return;	}
	if( !IS_START(m_DuelChannel[nId].nStatus) ) { sLog->outBasic("%s\t%s\t%s\t%s\t%d","IS_START(m_DuelChannel[nId].nStatus)", "0","NULL",__FILE__, __LINE__); return; }
    Obj.m_btDuelScore++;
}

void CNewPVP::CheckScore(CGameObject &Obj, CGameObject &target)
{
	int nId = GetDuelChannelId(Obj.m_Index);
	if(nId < 0 || nId >= DUEL_CHANNEL_MAX){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","nId>=0 && nId<DUEL_CHANNEL_MAX","0","NULL",__FILE__, __LINE__); return; }
	if( !IS_START(m_DuelChannel[nId].nStatus) ) { sLog->outBasic("%s\t%s\t%s\t%s\t%d","IS_START(m_DuelChannel[nId].nStatus)", "0","NULL",__FILE__, __LINE__); return; }

	LPDUEL_CHANNEL lpChannel = &m_DuelChannel[nId];

	CGameObject &requester = *getGameObject(lpChannel->nIndex1);
	CGameObject &responsor = *getGameObject(lpChannel->nIndex2);

	if( Obj.m_btDuelScore >= 10 )
    {
		PMSG_NOTICE pNotice;
		BroadcastDuelInfo( nId, 2);
        BroadcastScore( nId, 3);
        SetStatus(DC_RESERVEDEND, nId, requester, responsor);
        BroadcastResult( nId, 3, Obj);
        
		gObjAddBuffEffect(Obj, BUFFTYPE_GLORYOFGLADIATOR);

		gDarkSpirit[requester.m_Index-g_ConfigRead.server.GetObjectStartUserIndex()].ReSetTarget(responsor);
        gDarkSpirit[responsor.m_Index-g_ConfigRead.server.GetObjectStartUserIndex()].ReSetTarget(requester);
		g_CMuunSystem.ReSetTarget(requester, responsor);
		g_CMuunSystem.ReSetTarget(responsor, requester);
        Obj.WinDuels++;
		target.LoseDuels++;

		TNotice::MakeNoticeMsgEx(&pNotice, 0, Lang.GetText(0,310), Obj.Name, target.Name);
		TNotice::SendNoticeToAllUser(&pNotice);

		sLog->outBasic("[NewPVP] [%s][%s] Win Duel, Loser [%s][%s]", Obj.m_PlayerData->ConnectUser->AccountID, Obj.Name, target.m_PlayerData->ConnectUser->AccountID, target.Name);
		return;
	}

	BroadcastScore(nId, 3);
    SetStatus(4, nId, requester, responsor);
}

// UPTO 

void CNewPVP::ReFill(CGameObject &Obj)
{
	if( !gObjIsConnected(Obj) ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","gObjIsConnected(&Obj)","0","NULL", __FILE__, __LINE__); return; }

	Obj.BP = Obj.MaxBP + Obj.AddBP;
	Obj.Mana = Obj.MaxMana + Obj.AddMana;
	GSProtocol.GCManaSend(&Obj, (WORD)Obj.Mana, 0xFF, 0, (WORD)Obj.BP);

	Obj.Life = Obj.MaxLife + Obj.AddLife;
	Obj.iShield = Obj.iMaxShield + Obj.iAddShield;
	GSProtocol.GCReFillSend(&Obj, Obj.Life, 0xFF, 0, Obj.iShield);
}

BOOL CNewPVP::MoveGate(CGameObject &Obj, int nGateNum)
{
	BYTE btMapNum = MAP_INDEX_VULCANROOM;
	BYTE btDir;
	short sX;
	short sY;
	short sLevel;
	int nMapNumber;

	nMapNumber =  gGateC.GetGate(nGateNum, sX, sY, btMapNum, btDir, sLevel);
	if( nMapNumber < 0)	return FALSE;

	gObjTeleport(Obj, btMapNum, sX, sY);
	gObjViewportListProtocolCreate(Obj);
	return TRUE;
}

BOOL CNewPVP::CheckLimitLevel(CGameObject &Obj, int nGateNum)
{
	BYTE btMapNum = MAP_INDEX_VULCANROOM;
	BYTE btDir;
	short sX;
	short sY;
	short sLevel;

	int nMapNumber =  gGateC.GetGate(nGateNum, sX, sY, btMapNum, btDir, sLevel);
	if( nMapNumber < 0)	return FALSE;

	if(Obj.Level < sLevel)	return FALSE;
	return TRUE;
}

void CNewPVP::SetBuff(CGameObject &Obj, BOOL bUse)
{
	if( bUse )
	{
		gObjAddBuffEffect(Obj, BUFFTYPE_INVISABLE, 0, 0, 0, 0, -10);
		gObjAddBuffEffect(Obj, BUFFTYPE_OBSERVER, 0, 0, 0, 0, -10);
		gObjViewportListProtocolDestroy(Obj);
		return;
	}

	gObjRemoveBuffEffect(Obj, BUFFTYPE_INVISABLE);
	gObjRemoveBuffEffect(Obj, BUFFTYPE_OBSERVER);
	gObjViewportListProtocolCreate(Obj);
}

BOOL CNewPVP::SendChannelList(CGameObject &npc, CGameObject &Obj)
{
	GIOCP.DataSend(Obj.m_Index, (BYTE*)&m_DuelChannelList, m_DuelChannelList.h.size);
	return FALSE;
}

int CNewPVP::GetChannelStatus(CGameObject &Obj)
{
	DUEL_CHANNEL * lpChannel = GetDuelChannel(Obj.Name);
	if(lpChannel != NULL)	return lpChannel->nStatus;
	return 0;
}

BOOL CNewPVP::IsSafeState(CGameObject &Obj)
{
	DUEL_CHANNEL * lpChannel = GetDuelChannel(Obj.Name);
	if(lpChannel == NULL)	return TRUE;
	
	if( lpChannel->nStatus == 2 || 
		lpChannel->nStatus == 4 || 
		lpChannel->nStatus == DC_RESERVEDEND )
	{
		return TRUE;
	}

	return FALSE;
}

int CNewPVP::IsObserver(CGameObject &Obj)
{
	return gObjCheckUsedBuffEffect(Obj, BUFFTYPE_OBSERVER);
}

int CNewPVP::GetChannelIdByObserver(CGameObject &Obj)
{
	////EnterCriticalSection(&this->m_csObserver);

	std::map<int,ObserverInfo>::iterator iter = m_ObserverInfoList.find(Obj.m_Index);
	if(!(iter!=m_ObserverInfoList.end()))
	{
		////LeaveCriticalSection(&this->m_csObserver);
		return -1;
	}

	ObserverInfo & info = iter->second;

	if( info.nId < 0 || info.nId >= DUEL_CHANNEL_MAX)
    {
		sLog->outBasic("%s\t%s\t%s\t%s\t%d","info.nId>=0 && info.nId<DUEL_CHANNEL_MAX","-1","NULL",__FILE__, __LINE__);
		////LeaveCriticalSection(&this->m_csObserver);
		return -1;
    }

	int nId = info.nId;

	////LeaveCriticalSection(&this->m_csObserver);
	return nId;
}

int CNewPVP::CheckUsersOnConnect(int nId)
{  
	if(nId < 0 || nId >= DUEL_CHANNEL_MAX){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","nId>=0 && nId<DUEL_CHANNEL_MAX","FALSE","NULL",__FILE__, __LINE__); return FALSE; }
	if( !IS_START(m_DuelChannel[nId].nStatus) ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","IS_START(m_DuelChannel[nId].nStatus)", "FALSE","NULL",__FILE__, __LINE__); return FALSE; }

	CGameObject &requester = *getGameObject(m_DuelChannel[nId].nIndex1);
	CGameObject &responsor = *getGameObject(m_DuelChannel[nId].nIndex2);

	if( !gObjIsConnected(requester) || !IsPKFieldMap( requester.MapNumber) ){ Leave(requester); return FALSE; }
	if( !gObjIsConnected(responsor) || !IsPKFieldMap( responsor.MapNumber) ){ Leave(responsor); return FALSE; }
	return TRUE;
}

void CNewPVP::ChatMsgSend(CGameObject &Obj,BYTE* Msg, int size)
{
	int nId = -1;

	if( IsDuel(Obj) == TRUE )
	{
		nId = GetDuelChannelId(Obj.m_Index);
	}

	else nId = GetChannelIdByObserver(Obj);
	if(nId < 0 || nId >= DUEL_CHANNEL_MAX){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","nId>=0 && nId<DUEL_CHANNEL_MAX","0","NULL",__FILE__, __LINE__); return; }
	if(m_DuelChannel[nId].nStatus == DC_IDLE){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","m_DuelChannel[nId].nStatus!=DC_IDLE","0","NULL",__FILE__, __LINE__); return; }

	LPDUEL_CHANNEL lpChannel = &m_DuelChannel[nId];

	CGameObject &requester = *getGameObject(lpChannel->nIndex1);
    CGameObject &responsor = *getGameObject(lpChannel->nIndex2);

	if(gObjIsConnected(requester))
	{
		GIOCP.DataSend(requester.m_Index, Msg, size);
	}

	if(gObjIsConnected(responsor))
	{
		GIOCP.DataSend(responsor.m_Index, Msg, size);
	}

	BroadcastToObserver(nId, Msg, size);
}

void CNewPVP::Reset(CGameObject &Obj)
{
	if( IsDuelRequested(Obj) )	Cancel(Obj, *getGameObject(Obj.m_iDuelUserRequested), TRUE);
	if( IsDuel(Obj) )	Leave(Obj);

	if( IsObserver(Obj) )
	{
		////EnterCriticalSection(&this->m_csObserver);
		std::map<int,ObserverInfo>::iterator iter = m_ObserverInfoList.find(Obj.m_Index);

		if(!(iter!=m_ObserverInfoList.end()))
		{
			////LeaveCriticalSection(&this->m_csObserver);
			return;
		}

		ObserverInfo & info = iter->second;
		int nId = info.nId;

		////LeaveCriticalSection(&this->m_csObserver);
		LeaveChannel(nId, Obj);
	}
}

void CNewPVP::GetObserverList(int nId, PMSG_DUEL_OBSERVERLIST_BROADCAST& res)
{
	int i = 0;
	////EnterCriticalSection(&this->m_csObserver);

	for(std::map<int,ObserverInfo>::iterator iter = m_ObserverInfoList.begin();iter!=m_ObserverInfoList.end(); iter++)
	{
		ObserverInfo & info = iter->second;		
		if(info.nId == nId)
		{
			std::memcpy(&res.user[i], info.szName, MAX_ACCOUNT_LEN);
			i++;
			if( i >= 10 )	break;
		}
	}

	res.nCount = i;
	////LeaveCriticalSection(&this->m_csObserver);
}

int CNewPVP::JoinChannel(int nId,CGameObject &Obj)
{
	if( IsDuel(Obj) ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","!IsDuel(Obj)","ENEWPVP::E_ALREADY_DUELLING", "NULL", __FILE__,  __LINE__); return ENEWPVP::E_ALREADY_DUELLING; }
	if( IsDuelRequested(Obj) ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","!IsDuelRequested(Obj)","ENEWPVP::E_ALREADY_DUELREQUESTED","NULL",  __FILE__,  __LINE__); return ENEWPVP::E_ALREADY_DUELREQUESTED; }
	if( IsDuelReserved(Obj) ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","!IsDuelReserved(Obj)","ENEWPVP::E_ALREADY_DUELRESERVED","NULL", __FILE__,  __LINE__); return ENEWPVP::E_ALREADY_DUELRESERVED; }
	if(nId < 0 || nId >= DUEL_CHANNEL_MAX){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","nId>=0 && nId<DUEL_CHANNEL_MAX","ENEWPVP::E_INVALID_CHANNELID","NULL",__FILE__, __LINE__); return ENEWPVP::E_INVALID_CHANNELID; }
	if( !IS_START(m_DuelChannel[nId].nStatus) ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","IS_START(m_DuelChannel[nId].nStatus)", "ENEWPVP::E_INVALID_STATUS","NULL",__FILE__, __LINE__); return ENEWPVP::E_INVALID_STATUS; }
	if( m_ObserverCount[nId] + 1 > 10 )	return ENEWPVP::E_OBSERVER_MAX;
	if( !CheckLimitLevel(Obj, g_GateObserver[nId]) )	return ENEWPVP::E_LIMIT_LEVEL;

	////EnterCriticalSection(&this->m_csObserver);
	std::map<int,ObserverInfo>::iterator iter = m_ObserverInfoList.find(Obj.m_Index); 

	if( !(iter==m_ObserverInfoList.end()) )
	{
		////LeaveCriticalSection(&this->m_csObserver);
		return ENEWPVP::E_FAILED_ENTER;
	}

	////LeaveCriticalSection(&this->m_csObserver);

	CGameObject  &requester = *getGameObject(m_DuelChannel[nId].nIndex1);
    CGameObject  &responsor = *getGameObject(m_DuelChannel[nId].nIndex2);
	
	BroadcastJoinChannel(nId, Obj);

	ObserverInfo info = {0};
	info.nId = nId;
    info.nIndex = Obj.m_Index;
	std::memcpy(info.szName, Obj.Name, MAX_ACCOUNT_LEN);

	////EnterCriticalSection(&this->m_csObserver);
	std::pair< std::map<int,ObserverInfo>::iterator, bool > pair = m_ObserverInfoList.insert( std::make_pair(Obj.m_Index,  info) );

	if(pair.second == false)
	{
		////LeaveCriticalSection(&this->m_csObserver);
		return ENEWPVP::E_FAILED_ENTER;
	}

	m_ObserverCount[nId]++;
	////LeaveCriticalSection(&this->m_csObserver);

	PMSG_ANS_DUEL_JOINCNANNEL res = {0};
	res.h.c = 0xC1;
    res.h.headcode = 0xAA;
    res.h.subcode = 0x07;
    res.h.size = sizeof(PMSG_ANS_DUEL_JOINCNANNEL);
	res.nResult = 0x00;
    res.nChannelId = nId;
	res.NumberH1 = HIBYTE(requester.m_Index);
    res.NumberL1 = LOBYTE(requester.m_Index);
    res.NumberH2 = HIBYTE(responsor.m_Index);
    res.NumberL2 = LOBYTE(responsor.m_Index);
    std::memcpy(res.szName1, requester.Name, MAX_ACCOUNT_LEN);
    std::memcpy(res.szName2, responsor.Name, MAX_ACCOUNT_LEN);
    GIOCP.DataSend(Obj.m_Index, (BYTE*)&res,res.h.size);

	PMSG_DUEL_OBSERVERLIST_BROADCAST Msg = {0};
	Msg.h.c = 0xC1;
	Msg.h.headcode = 0xAA;
	Msg.h.subcode = 0x0B;
	Msg.h.size = sizeof(PMSG_DUEL_OBSERVERLIST_BROADCAST);

	GetObserverList(nId, Msg);

	GIOCP.DataSend(Obj.m_Index, (BYTE*)&Msg, Msg.h.size);

	if( Msg.nCount!=m_ObserverCount[nId] )
    {
		sLog->outBasic("[NewPVP] ChannelId[%d] Count[%d][%d]", nId, Msg.nCount, m_ObserverCount[nId]);
		
		sLog->outBasic("[NewPVP] [Debug] %s(%d):",Obj.Name,Msg.nCount);
		for(int n = 0; n < Msg.nCount; n++)
		{
			sLog->outBasic("[NewPVP] [Debug] %s,",Msg.user[n].szName);
		}
	}

	CheckChannelListInfo( nId);
    BroadcastDuelInfo(nId, 2);
    BroadcastScore(nId, 2);
    SetBuff(Obj, TRUE);
    MoveGate(Obj, g_GateObserver[nId]);
	return 0;
}

int CNewPVP::LeaveChannel(int nId,CGameObject &Obj)
{
	if(nId < 0 || nId >= DUEL_CHANNEL_MAX){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","nId>=0 && nId<DUEL_CHANNEL_MAX","ENEWPVP::E_INVALID_CHANNELID","NULL",__FILE__, __LINE__); return ENEWPVP::E_INVALID_CHANNELID; }
	if( !IS_START(m_DuelChannel[nId].nStatus) ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","IS_START(m_DuelChannel[nId].nStatus)", "ENEWPVP::E_INVALID_STATUS","NULL",__FILE__, __LINE__); return ENEWPVP::E_INVALID_STATUS; }	

	//EnterCriticalSection(&this->m_csObserver);
	std::map<int,ObserverInfo>::iterator iter = m_ObserverInfoList.find(Obj.m_Index); 

	if(!(iter!=m_ObserverInfoList.end()))
	{
		//LeaveCriticalSection(&this->m_csObserver);
		return ENEWPVP::E_NOT_EXIST_USER;
	}

	m_ObserverInfoList.erase(iter);
	m_ObserverCount[nId]--;
	//LeaveCriticalSection(&this->m_csObserver);

	BroadcastLeaveChannel( nId, Obj);

	if( gObjIsConnected(Obj) )
    {
		PMSG_ANS_DUEL_LEAVECNANNEL res = {0};
		res.h.c = 0xC1;
		res.h.headcode = 0xAA;
		res.h.subcode = 0x09;
		res.h.size = sizeof(PMSG_ANS_DUEL_LEAVECNANNEL);
		res.nResult = 0x00;		
		GIOCP.DataSend(Obj.m_Index, (BYTE*)&res, res.h.size);
          
		if( IsPKFieldMap(Obj.MapNumber) )
		{
			if (Obj.Level < gGateC.GetLevel(294))
			{
				if (Obj.Class == CLASS_ELF)
				{
					MoveGate(Obj, 27);
				}

				else if(Obj.Class == CLASS_SUMMONER)
				{
					MoveGate(Obj, 267);
				}

				else
				{
					MoveGate(Obj, 17);
				}
			}

			else
			{
				MoveGate(Obj, 294);
			}
		}

		SetBuff(Obj, FALSE);
    }

	CheckChannelListInfo( nId);
	return 0;
}

void CNewPVP::LeaveChannelObserver(int nId)
{
	//EnterCriticalSection(&this->m_csObserver);
	std::map<int,ObserverInfo>::iterator iter = m_ObserverInfoList.begin();

	while(iter!=m_ObserverInfoList.end())
	{
		ObserverInfo & info = iter->second;		
		if(info.nId == nId)
		{
			CGameObject &Obj = *getGameObject(info.nIndex);

			if( IsPKFieldMap(Obj.MapNumber) )
			{
				if (Obj.Level < gGateC.GetLevel(294))
				{
					if (Obj.Class == CLASS_ELF)
					{
						MoveGate(Obj, 27);
					}

					else if(Obj.Class == CLASS_SUMMONER)
					{
						MoveGate(Obj, 267);
					}

					else
					{
						MoveGate(Obj, 17);
					}
				}

				else
				{
					MoveGate(Obj, 294);
				}
			}
			
			SetBuff(Obj, FALSE);
			iter = m_ObserverInfoList.erase(iter);
			continue;
		}		
		iter++;
	}

	m_ObserverCount[nId] = 0;
	//LeaveCriticalSection(&this->m_csObserver);
}

void CNewPVP::CheckChannelListInfo(int nId)
{
	if( nId < 0 || nId >= DUEL_CHANNEL_MAX )	return;	
	m_DuelChannelList.channel[nId].bWatch = 1;
}

void CNewPVP::BroadcastScore(int nId, BYTE nFlag)
{
	if( nId < 0 || nId >= DUEL_CHANNEL_MAX ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","nId>=0 && nId<DUEL_CHANNEL_MAX","0","NULL",__FILE__, __LINE__); return; }
	if( !IS_START(m_DuelChannel[nId].nStatus) ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","IS_START(m_DuelChannel[nId].nStatus)", "0","NULL",__FILE__, __LINE__); return; }

	CGameObject &requester = *getGameObject(m_DuelChannel[nId].nIndex1);
    CGameObject &responsor = *getGameObject(m_DuelChannel[nId].nIndex2);

	PMSG_DUEL_SCORE_BROADCAST res = {0};
	res.h.c = 0xC1;
	res.h.headcode = 0xAA;
	res.h.subcode = 0x04;
	res.h.size = sizeof(PMSG_DUEL_SCORE_BROADCAST);
	res.NumberH1 = HIBYTE(requester.m_Index);
    res.NumberL1 = LOBYTE(requester.m_Index);
    res.NumberH2 = HIBYTE(responsor.m_Index);
    res.NumberL2 = LOBYTE(responsor.m_Index);
    res.btDuelScore1 = requester.m_btDuelScore;
    res.btDuelScore2 = responsor.m_btDuelScore;
      
	if( (nFlag & 1) == 1 )
    {
		GIOCP.DataSend(requester.m_Index, (BYTE*)&res, res.h.size);
        GIOCP.DataSend(responsor.m_Index, (BYTE*)&res, res.h.size);
    }
	if( (nFlag & 2) == 2 )
	{
        BroadcastToObserver(nId, (BYTE*)&res, res.h.size);
	}
}

void CNewPVP::BroadcastResult(int nId, BYTE nFlag, CGameObject &Obj)
{
	if( nId < 0 || nId >= DUEL_CHANNEL_MAX ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","nId>=0 && nId<DUEL_CHANNEL_MAX","0","NULL",__FILE__, __LINE__); return; }
	if( m_DuelChannel[nId].nStatus != DC_RESERVEDEND ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","m_DuelChannel[nId].nStatus==DC_RESERVEDEND", "0","NULL",__FILE__, __LINE__); return;	}
	if( !gObjIsConnected(Obj) ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","gObjIsConnected(Obj.m_Index)", "0","NULL",__FILE__, __LINE__); return; }
	if( !IsDuel(Obj) ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","IsDuel(Obj)", "0","NULL",__FILE__, __LINE__); return; }

	CGameObject &targetObj = *getGameObject(Obj.m_iDuelUser);
     
	PMSG_DUEL_RESULT_BROADCAST res = {0};
	res.h.c = 0xC1;
	res.h.headcode = 0xAA;
	res.h.subcode = 0x0C;
	res.h.size = sizeof(PMSG_DUEL_RESULT_BROADCAST);
	std::memcpy(res.szWinner, Obj.Name, MAX_ACCOUNT_LEN);
	std::memcpy(res.szLoser, targetObj.Name, MAX_ACCOUNT_LEN);
	
	if( (nFlag & 1) == 1 )
	{
		GIOCP.DataSend(Obj.m_Index, (BYTE*)&res, res.h.size);
		GIOCP.DataSend(targetObj.m_Index, (BYTE*)&res, res.h.size);
    }
	if( (nFlag & 2) == 2 )
	{
		BroadcastToObserver(nId, (BYTE*)&res,res.h.size);
	}
}

void CNewPVP::BroadcastDuelInfo(int nId, BYTE nFlag)
{
	if( nId < 0 || nId >= DUEL_CHANNEL_MAX ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","nId>=0 && nId<DUEL_CHANNEL_MAX","0","NULL",__FILE__, __LINE__); return; }
	if( !IS_START(m_DuelChannel[nId].nStatus) ) return;

	CGameObject &requester = *getGameObject(m_DuelChannel[nId].nIndex1);
    CGameObject &responsor = *getGameObject(m_DuelChannel[nId].nIndex2);
	
	int nLifePer1 = 0;
    int nLifePer2 = 0;
    int ShieldPer1 = 0;
    int ShieldPer2 = 0;

	if(requester.AddLife +  requester.MaxLife <  requester.Life)
	{
		 requester.Life =  requester.AddLife +  requester.MaxLife;
	}

	if(requester.iShield > requester.iMaxShield + requester.iAddShield)
	{
		requester.iShield = requester.iMaxShield + requester.iAddShield;
	}

	if(responsor.AddLife +  responsor.MaxLife <  responsor.Life)
	{
		 responsor.Life =  responsor.AddLife +  responsor.MaxLife;
	}

	if(responsor.iShield > responsor.iMaxShield + responsor.iAddShield)
	{
		responsor.iShield = responsor.iMaxShield + responsor.iAddShield;
	}
	
	nLifePer1 = (requester.Life / ((float)(requester.AddLife) + requester.MaxLife) * 100.0f);
	nLifePer2 = (responsor.Life / ((float)(responsor.AddLife) + responsor.MaxLife) * 100.0f);
	ShieldPer1 = ((float)(requester.iShield) / (float)((int)(requester.iMaxShield) + (int)(requester.iAddShield)) * 100.0f);
	ShieldPer2 = ((float)(responsor.iShield) / (float)((int)(responsor.iMaxShield) + (int)(responsor.iAddShield)) * 100.0f);

	PMSG_DUEL_HP_BROADCAST res = {0};
	res.h.c = 0xC1;
	res.h.headcode = 0xAA;
	res.h.subcode = 0x05;
	res.h.size = sizeof(PMSG_DUEL_HP_BROADCAST);
	res.NumberH1 = HIBYTE(requester.m_Index);
    res.NumberL1 = LOBYTE(requester.m_Index);
	res.NumberH2 = HIBYTE(responsor.m_Index);
    res.NumberL2 = LOBYTE(responsor.m_Index);
	res.nHP1 = nLifePer1;
	res.nHP2 = nLifePer2;
	res.nShield1 = ShieldPer1;
	res.nShield2 = ShieldPer2;
	
	if( (nFlag & 1) == 1 )
	{
		GIOCP.DataSend(requester.m_Index, (BYTE*)&res, res.h.size);
		GIOCP.DataSend(responsor.m_Index, (BYTE*)&res, res.h.size);
	}
	if( (nFlag & 2) == 2 )
	{
		BroadcastToObserver(nId, (BYTE*)&res, res.h.size);
	}
}

void CNewPVP::BroadcastJoinChannel(int nId, CGameObject &Obj)
{
	if( nId < 0 || nId >= DUEL_CHANNEL_MAX ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","nId>=0 && nId<DUEL_CHANNEL_MAX","0","NULL",__FILE__, __LINE__); return; }
	if( !IS_START(m_DuelChannel[nId].nStatus) ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","IS_START(m_DuelChannel[nId].nStatus)","0","NULL",__FILE__, __LINE__); return; }

	PMSG_DUEL_JOINCNANNEL_BROADCAST res = {0};
	res.h.c = 0xC1;
    res.h.headcode = 0xAA;
    res.h.subcode = 0x08;
	res.h.size = sizeof(PMSG_DUEL_JOINCNANNEL_BROADCAST);
    std::memcpy(res.szName, Obj.Name, MAX_ACCOUNT_LEN);
    BroadcastToObserver(nId, (BYTE*)&res, res.h.size);
}

void CNewPVP::BroadcastLeaveChannel(int nId, CGameObject &Obj)
{
	if( nId < 0 || nId >= DUEL_CHANNEL_MAX ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","nId>=0 && nId<DUEL_CHANNEL_MAX","0","NULL",__FILE__, __LINE__); return; }
	if( !IS_START(m_DuelChannel[nId].nStatus) ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","IS_START(m_DuelChannel[nId].nStatus)","0","NULL",__FILE__, __LINE__); return; }

	PMSG_DUEL_LEAVECNANNEL_BROADCAST res = {0};
	res.h.c = 0xC1;
    res.h.headcode = 0xAA;
    res.h.subcode = 0x0A;
	res.h.size = sizeof(PMSG_DUEL_LEAVECNANNEL_BROADCAST);
    std::memcpy(res.szName, Obj.Name, MAX_ACCOUNT_LEN);
    BroadcastToObserver(nId, (BYTE*)&res,res.h.size);
}

void CNewPVP::BroadcastToObserver( int nId, BYTE *lpData, int nSize)
{
	int nSendCount = 0;
	int SendIndex[10];
	memset(SendIndex, 0x00, sizeof(SendIndex));

	//EnterCriticalSection(&this->m_csObserver);

	for(std::map<int,ObserverInfo>::iterator iter = m_ObserverInfoList.begin();iter!=m_ObserverInfoList.end(); iter++)
	{
		ObserverInfo &info = iter->second;		
		if(info.nId == nId)
		{
			//if( gObjIsConnected(info.nIndex) )
			//{
				SendIndex[nSendCount] = info.nIndex;
				nSendCount++;
			//}
		}
	}

	//LeaveCriticalSection(&this->m_csObserver);

	for (int i = 0; i < nSendCount; i++)
	{
		GIOCP.DataSend(SendIndex[i], lpData, nSize);
	}
}

void CNewPVP::BroadcastMessage( int nId, BYTE nFlag, BYTE nMsgType, int nNotifySec)
{
	if( nId < 0 || nId >= DUEL_CHANNEL_MAX ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","nId>=0 && nId<DUEL_CHANNEL_MAX","0","NULL",__FILE__, __LINE__); return; }
	if( nNotifySec <= 0 )	return;

	CGameObject &requester = *getGameObject(m_DuelChannel[nId].nIndex1);
    CGameObject &responsor = *getGameObject(m_DuelChannel[nId].nIndex2);

	PMSG_NOTICE res = {0} ;

	switch(nMsgType)
	{
	case 0:	TNotice::MakeNoticeMsgEx(&res, 1, Lang.GetText(0,323), nNotifySec);	break;
	case 1:	TNotice::MakeNoticeMsgEx(&res, 1, Lang.GetText(0,324), nNotifySec);	break;
	case 2:	TNotice::MakeNoticeMsgEx(&res, 1, Lang.GetText(0,325), nNotifySec);	break;
	default:	return;
	}

	if( (nFlag & 1) == 1 )
    {
		if( gObjIsConnected(requester) )	GIOCP.DataSend(requester.m_Index, (BYTE*)&res,res.h.size);
		if( gObjIsConnected(responsor) )	GIOCP.DataSend(responsor.m_Index, (BYTE*)&res,res.h.size);
    }
	if( (nFlag & 2) == 2 )
	{
      BroadcastToObserver(nId, (BYTE*)&res, res.h.size);
	}
}

void CNewPVP::BroadcastRound(int nId, BYTE nFlag, BOOL bEnd)
{
	if( nId < 0 || nId >= DUEL_CHANNEL_MAX ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","nId>=0 && nId<DUEL_CHANNEL_MAX","0","NULL",__FILE__, __LINE__); return; }

	CGameObject &requester = *getGameObject(m_DuelChannel[nId].nIndex1);
    CGameObject &responsor = *getGameObject(m_DuelChannel[nId].nIndex2);

	PMSG_DUEL_ROUNDSTART_BROADCAST res = {0}; 
    res.h.c = 0xC1;
    res.h.headcode = 0xAA;
    res.h.subcode = 0x0D;
    res.h.size = sizeof(PMSG_DUEL_ROUNDSTART_BROADCAST);
    res.nFlag = bEnd;

    if( (nFlag & 1) == 1 )
    {
		GIOCP.DataSend(requester.m_Index,(BYTE*)&res,res.h.size);
		GIOCP.DataSend(responsor.m_Index, (BYTE*)&res, res.h.size);
    }
    if( (nFlag & 2) == 2 )
	{
		BroadcastToObserver(nId, (BYTE*)&res,res.h.size);
	}
}

BOOL CNewPVP::CheckPKPenalty(CGameObject &Obj)
{
	if( !g_ConfigRead.pk.bPkPenaltyDisable )
	{
		if( Obj.PartyNumber >= 0 )
		{
			if( gParty.GetPKPartyPenalty( Obj.PartyNumber) >= 6 )
			{
				return TRUE;
			}
		}

		else if( Obj.m_PK_Level >= 6 )
		{
			 return TRUE;
		}
    }
	return FALSE;
}

BOOL CNewPVP::IsSelfDefense(CGameObject &Obj)
{
	BOOL bRetVal = FALSE;
	for ( int i = 0; i < MAX_SELF_DEFENSE; i++ )
	{
		if( Obj.SelfDefense[i] >= 0 )
		{
			bRetVal = TRUE;
			break;
		}
	}
	return bRetVal;
}

BOOL CNewPVP::IsGuildWar(CGameObject &Obj)
{
	if( Obj.m_PlayerData->lpGuild && 
		Obj.m_PlayerData->lpGuild->WarState == 1)
	{
		return TRUE;
	}
	return FALSE;
}

BOOL CNewPVP::DropItem(CGameObject &Obj, CGameObject &MonsterObj)
{
	if( !IsVulcanusMap(Obj.MapNumber) ){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","IsVulcanusMap(Obj.MapNumber)","FALSE","NULL",__FILE__,  __LINE__); return FALSE; }
	if( m_bNewPVP != TRUE){ sLog->outBasic("%s\t%s\t%s\t%s\t%d","m_bNewPVP==TRUE","FALSE","NULL", __FILE__,  __LINE__); return FALSE; }
	if( !gObjCheckUsedBuffEffect(Obj, BUFFTYPE_GLORYOFGLADIATOR) )	return FALSE;
	if( !gObjGetTotalValueOfEffect(Obj, EFFECTTYPE_VULCANUS_ITEMDROPRATE) )	return FALSE;

	if (g_BagManager.SearchAndUseBag(Obj, BAG_EVENT, EVENTBAG_NEWPVP, MonsterObj.m_Index) == false)
	{
		return FALSE;
	}

	return TRUE;
}


