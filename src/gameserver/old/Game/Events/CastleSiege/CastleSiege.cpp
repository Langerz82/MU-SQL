...
void CCastleSiege::ClearDbNPC()
{
	...
	{
		if( user.second->MapNumber == MAP_INDEX_CASTLESIEGE )
		{
			if( user.second->Class == 277 || user.second->Class == 283 || user.second->Class == 219 )
			{
				if( user.second->Class == 277 )
				{
					this->SetGateBlockState(user.second->X, user.second->Y, 1);
					gObjAddBuffEffect(*user.second, BUFFTYPE_CASTLE_DOOR_STATE, 0, 0, 0, 0, -10);
				}
				gObjDel(*user.second);
			}
		}
...
void CCastleSiege::ClearNonDbNPC()
{
	for each (std::pair<int, CGameObject*> user in gGameObjects)
	{
		if(gObjIsConnected(*user.second) && user.second->MapNumber == MAP_INDEX_CASTLESIEGE	&& user.second->m_btCsNpcType != NULL && user.second->m_btCsNpcType != 1)
		{
			if(user.second->Class == 278)
			{
				g_CsNPC_LifeStone.DeleteLifeStone(n);
			}

			if(user.second->Class == 287 || user.second->Class == 286)
			{
				g_CsNPC_Mercenary.DeleteMercenary(n);
			}

			if(user.second->Class == 219)
			{
				continue;
			}
...
void CCastleSiege::ClearNonDbNPC_MidWin()
{
	for each (std::pair<int, CGameObject*> user in gGameObjects)
	{
		if( gObjIsConnected(*user.second) )
		{
			if( user.second->MapNumber == MAP_INDEX_CASTLESIEGE )
			{
				if( user.second->m_btCsNpcType && user.second->m_btCsNpcType != 1 )
				{
					if( user.second->Class == 278 )
					{
						g_CsNPC_LifeStone.DeleteLifeStone(n);
...
void CCastleSiege::CreateDbNPC()
{
	...
	while(it != this->m_vtNpcData.end())
	{
		...
		if(pNpcData.m_bIN_USE == 1 && pNpcData.m_iNPC_LIVE == 1)
		{
			...
			CGameObject* lpObjNPC = getGameObject(iNPC_INDEX);

			if(iNPC_INDEX >= 0)
			{
				gObjSetMonster(*lpObjNPC, pNpcData.m_iNPC_NUM);
				lpObjNPC->m_PosNum = -1;
				lpObjNPC->X = pNpcData.m_iNPC_SX;
				lpObjNPC->Y = pNpcData.m_iNPC_SY;
				lpObjNPC->MapNumber = MAP_INDEX_CASTLESIEGE;
				lpObjNPC->TX = lpObjNPC->X;
				lpObjNPC->TY = lpObjNPC->Y;
				lpObjNPC->m_OldX = lpObjNPC->X;
				lpObjNPC->m_OldY = lpObjNPC->Y;
				lpObjNPC->Dir = pNpcData.m_iNPC_DIR;
				lpObjNPC->StartX = lpObjNPC->X;
				lpObjNPC->StartY = lpObjNPC->Y;
				lpObjNPC->DieRegen = 0;
				lpObjNPC->MaxRegenTime = 0;
				lpObjNPC->Life = pNpcData.m_iNPC_HP;
				lpObjNPC->MaxLife = pNpcData.m_iNPC_MAXHP;
				lpObjNPC->m_btCsNpcType = 1;
				...
				{
				case 277:
					lpObjNPC->m_Defense = g_iNpcDefense_CGATE[iNPC_DF_LEVEL);
					lpObjNPC->m_MagicDefense = g_iNpcDefense_CGATE[iNPC_DF_LEVEL);
					lpObjNPC->m_btCsNpcDfLevel = iNPC_DF_LEVEL;
					break;
				case 283:
					lpObjNPC->m_Defense = g_iNpcDefense_CSTATUE[iNPC_DF_LEVEL);
					lpObjNPC->m_MagicDefense = g_iNpcDefense_CSTATUE[iNPC_DF_LEVEL);
					lpObjNPC->m_btCsNpcDfLevel = iNPC_DF_LEVEL;
					lpObjNPC->m_btCsNpcRgLevel = iNPC_RG_LEVEL;
					break;
				}

				it->m_btCsNpcExistVal1 = rand()%255+1; //Check
				lpObjNPC->m_btCsNpcExistVal1 = it->m_btCsNpcExistVal1;

				it->m_btCsNpcExistVal2 = rand()%256;
				lpObjNPC->m_btCsNpcExistVal2 = it->m_btCsNpcExistVal2;

				it->m_btCsNpcExistVal3 = rand()%256;
				lpObjNPC->m_btCsNpcExistVal3 = it->m_btCsNpcExistVal3;

				it->m_btCsNpcExistVal4 = rand()%256;
				lpObjNPC->m_btCsNpcExistVal4 = it->m_btCsNpcExistVal4;

				if(pNpcData.m_iNPC_NUM == 277)
				{
					this->SetGateBlockState(pNpcData.m_iNPC_SX,pNpcData.m_iNPC_SY,1);
					gObjAddBuffEffect(*lpObjNPC, BUFFTYPE_CASTLE_DOOR_STATE, 0, 0, 0, 0, -10);

					lpObjNPC->m_btCsGateOpen = 1;
	...
	while(it != this->m_vtNpcData.end())
	{
		...
		if(pNpcData.m_bIN_USE == TRUE && pNpcData.m_bNPC_DBSAVE == FALSE)
		{
			...
			if(iNPC_INDEX >= 0)
			{
				...
				{
				case 1:
					lpObjNPC->m_btCsNpcType = 2;
					break;
				case 2:
					lpObjNPC->m_btCsNpcType = 3;
					break;
				default:
					lpObjNPC->m_btCsNpcType = 3;
					break;
...
int CCastleSiege::CheckAddDbNPC(CGameObject &Obj, int iNpcType, int iNpcIndex, BYTE & btResult)
{
	int iRETVAL = 0;
	...
	{
		_CS_NPC_DATA & NpcData = _CS_NPC_DATA(*it);
		CGameObject* lpObjNPC = getGameObject(NpcData.m_iNPC_OBJINDEX);

		if( NpcData.m_iNPC_NUM == iNpcType && NpcData.m_iNPC_INDEX == iNpcIndex )
		{
			if( NpcData.m_bIN_USE == TRUE && NpcData.m_iNPC_LIVE > 0 && lpObjNPC->Class == iNpcType )
			{
				if( lpObjNPC->Live == 1 && lpObjNPC->m_iCsNpcExistVal == NpcData.m_iCsNpcExistVal && lpObjNPC->m_iCsNpcExistVal != 0 )
				{
					btResult = 0;
					iRETVAL = 0;
					break;
			...
			if( Obj.m_PlayerData->Money < iNpcBuyCost )
			{
				btResult = 3;
				iRETVAL = 0;
				break;
			...
			GS_GDReqCastleNpcBuy(m_iMapSvrGroup, Obj.m_Index, iNpcType, iNpcIndex, NpcData.m_iNPC_DF_LEVEL, NpcData.m_iNPC_RG_LEVEL, NpcData.m_iNPC_MAXHP, NpcData.m_iNPC_HP, NpcData.m_iNPC_SX, NpcData.m_iNPC_SY, NpcData.m_iNPC_DIR, iNpcBuyCost);
			btResult = 1;
			iRETVAL = 1;
			break;
		}
...
int CCastleSiege::AddDbNPC(int iNpcType, int iNpcIndex)
{
	...
	while( it != this->m_vtNpcData.end() )
	{
		...
		if( (NpcData.m_iNPC_NUM == iNpcType) && (NpcData.m_iNPC_INDEX == iNpcIndex) )
		{
			...
			{
				if( lpObjNPC->Class == iNpcType )
				{
					if( lpObjNPC->Live == TRUE )
					{
						if( lpObjNPC->m_iCsNpcExistVal == NpcData.m_iCsNpcExistVal )
						{
							if( lpObjNPC->m_iCsNpcExistVal )
							{
								sLog->outBasic("[CastleSiege] CCastleSiege::AddDbNPC() ERROR - DB NPC EXIST (CLS:%d, IDX:%d(%d), MAXHP:%d, HP:%d, DF:%d, RG:%d)",	NpcData.m_iNPC_NUM, NpcData.m_iNPC_INDEX, NpcData.m_iNPC_OBJINDEX, getGameObject(NpcData.m_iNPC_OBJINDEX]->MaxLife, getGameObject(NpcData.m_iNPC_OBJINDEX]->Life, lpObjNPC->m_btCsNpcDfLevel, lpObjNPC->m_btCsNpcRgLevel);
								break;
							}
						}
					}
			...
			CGameObject *lpObjMonster = getGameObject(iObjIndex);

			if( iObjIndex >= 0 )
			{
				::gObjSetMonster(*lpObjMonster, NpcData.m_iNPC_NUM);
					
				lpObjMonster->m_PosNum = -1;
				lpObjMonster->X = NpcData.m_iNPC_SX;
				lpObjMonster->Y = NpcData.m_iNPC_SY;
				lpObjMonster->MapNumber = MAP_INDEX_CASTLESIEGE;
				...
				lpObjMonster->Dir = NpcData.m_iNPC_DIR;
				lpObjMonster->StartX = lpObjMonster->X;
				lpObjMonster->StartY = lpObjMonster->Y;
				lpObjMonster->DieRegen = 0;
				lpObjMonster->MaxRegenTime = 0;
					
				lpObjMonster->Life = NpcData.m_iNPC_BASE_HP;
				lpObjMonster->MaxLife = NpcData.m_iNPC_BASE_MAXHP;
				lpObjMonster->m_btCsNpcType = 1;
				...
				case 277:
					{
						lpObjMonster->m_Defense = g_iNpcDefense_CGATE[iBaseDFLevel);
						lpObjMonster->m_btCsNpcDfLevel = iBaseDFLevel;
					}
					break;
				case 283:
					{
						lpObjMonster->m_Defense = g_iNpcDefense_CSTATUE[iBaseDFLevel);
						lpObjMonster->m_btCsNpcDfLevel = iBaseDFLevel;
						lpObjMonster->m_btCsNpcRgLevel = iBaseRGLevel;
					}
					break;
				}

				it->m_btCsNpcExistVal1 = ( rand() % 255) + 1;
				lpObjMonster->m_btCsNpcExistVal1 = it->m_btCsNpcExistVal1;

				it->m_btCsNpcExistVal2 = ( rand() % 256);
				lpObjMonster->m_btCsNpcExistVal2 = it->m_btCsNpcExistVal2;

				it->m_btCsNpcExistVal3 = ( rand() % 256);
				lpObjMonster->m_btCsNpcExistVal3 = it->m_btCsNpcExistVal3;

				it->m_btCsNpcExistVal4 = ( rand() % 256);
				lpObjMonster->m_btCsNpcExistVal4 = it->m_btCsNpcExistVal4;

				if( NpcData.m_iNPC_NUM == 277 )
				{
					this->SetGateBlockState(NpcData.m_iNPC_SX, NpcData.m_iNPC_SY, 1);
					gObjAddBuffEffect(*lpObjMonster, BUFFTYPE_CASTLE_DOOR_STATE, 0, 0, 0, 0, -10);

					lpObjMonster->m_btCsGateOpen = 1;
...
BOOL CCastleSiege::DelNPC(CGameObject &Obj, int iNpcType, int iMonsterExistVal, int bDbSave)
{
	...
	while( it != this->m_vtNpcData.end() )
	{
		...
		if( pNpcData->m_iNPC_NUM == iNpcType && pNpcData->m_iNPC_OBJINDEX == Obj.m_Index)
		{
...
BOOL CCastleSiege::RepairDbNPC(int iNpcType, int iNpcIndex, int iNpcHP, int iNpcMaxHP)
{
	...
	while( it != this->m_vtNpcData.end() )
	{
		...
		if( NpcData.m_iNPC_NUM == iNpcType && NpcData.m_iNPC_INDEX == iNpcIndex && NpcData.m_bIN_USE == 1 && NpcData.m_iNPC_LIVE > 0 )
		{
			...
			CGameObject* lpObjNPC = getGameObject(iObjIndex);

			if(gObjIsConnected(*lpObjNPC) != FALSE && lpObjNPC->m_btCsNpcType != 0 && lpObjNPC->Class == iNpcType && lpObjNPC->m_iCsNpcExistVal == NpcData.m_iCsNpcExistVal && lpObjNPC->m_iCsNpcExistVal != 0 )
			{
...
int CCastleSiege::PayForUpgradeDbNpc(CGameObject &Obj, int iNpcType, int iNpcIndex, int iNpcUpType, int iNpcUpValue, int iNpcUpIndex)
{
	...
	case 277:
		{
			...
			case 1:
				{
					...
					{
						GSProtocol.GCAnsNpcUpgrade(&Obj, 3, iNpcType, iNpcIndex, iNpcUpType, iNpcUpValue);
						return FALSE;
					}

					int iDelResult = DelGemOfDefend(Obj, iNEED_GEMOFDEFEND);

					if( iDelResult == FALSE )
					{
						GSProtocol.GCAnsNpcUpgrade(&Obj, 4, iNpcType, iNpcIndex, iNpcUpType, iNpcUpValue);
						return FALSE;	
					}

					Obj.m_PlayerData->Money -= iNEED_MONEY;
				}
				break;
			case 3:
				{
			...
			default:
				{
					GSProtocol.GCAnsNpcUpgrade(&Obj, 5, iNpcType, iNpcIndex, iNpcUpType, iNpcUpValue);
					return FALSE;	
				}
				break;
			}
...
BOOL CCastleSiege::UpgradeDbNPC(CGameObject &Obj, int iNpcType, int iNpcIndex, int iNpcUpType, int iNpcUpValue, int iNpcUpIndex)
{
	if( CS_CHECK_UPGRADE(iNpcUpIndex) == FALSE )
	{
...
int CCastleSiege::UpgradeDbNPC_DFLEVEL(int iNpcType, int iNpcIndex, int iNpcDfLevel)
{
	...
	while( it != this->m_vtNpcData.end() )
	{
		...
		if( NpcData.m_iNPC_NUM == iNpcType && NpcData.m_iNPC_INDEX == iNpcIndex && NpcData.m_bIN_USE == 1 && NpcData.m_iNPC_LIVE > 0 )
		{
			...
			if( lpObjNPC->m_btCsNpcType != 0 &&	lpObjNPC->m_iCsNpcExistVal == NpcData.m_iCsNpcExistVal && lpObjNPC->m_iCsNpcExistVal != 0 )
			{
				it->m_iNPC_DF_LEVEL = iNpcDfLevel;
				
				bEXIST = TRUE;
	
				lpObjNPC->m_Defense = g_iNpcDefense_CGATE[iNpcDfLevel);
				lpObjNPC->m_MagicDefense = g_iNpcDefense_CGATE[iNpcDfLevel);
				lpObjNPC->m_btCsNpcDfLevel = iNpcDfLevel;
			}
			break;
		}
		it++;
	...
	while( it != this->m_vtNpcData.end() )
	{
		...
		if( NpcData.m_iNPC_NUM == iNpcType && NpcData.m_iNPC_INDEX == iNpcIndex && NpcData.m_bIN_USE == 1 && NpcData.m_iNPC_LIVE > 0 )
		{
			...
			if( lpObjNPC->m_btCsNpcType != 0 &&	lpObjNPC->m_iCsNpcExistVal == NpcData.m_iCsNpcExistVal && lpObjNPC->m_iCsNpcExistVal != 0 )
			{
				...
				bEXIST = TRUE;
				lpObjNPC->m_btCsNpcRgLevel = iNpcRgLevel;
			}
			break;
		}
		it++;
	...
	while( it != this->m_vtNpcData.end() )
	{
		...
		if( NpcData.m_iNPC_NUM == iNpcType && NpcData.m_iNPC_INDEX == iNpcIndex && NpcData.m_bIN_USE == 1 && NpcData.m_iNPC_LIVE > 0 )
		{
			...
			if( lpObjNPC->m_btCsNpcType != 0 &&	lpObjNPC->m_iCsNpcExistVal == NpcData.m_iCsNpcExistVal && lpObjNPC->m_iCsNpcExistVal != 0 )
			{
				...
				bEXIST = TRUE;
				lpObjNPC->Life = iNpcMaxHP;
				lpObjNPC->MaxLife = iNpcMaxHP;
			}
			break;
		}
		it++;
...
void CCastleSiege::StoreDbNpc()
{
	...
	while(it != this->m_vtNpcData.end())
	{
		...
		_CS_NPC_DATA & pNpcData = _CS_NPC_DATA(*it);
		CGameObject* lpObjNPC = getGameObject(pNpcData.m_iNPC_OBJINDEX);

		if(pNpcData.m_bNPC_DBSAVE == TRUE  && lpObjNPC->Class == pNpcData.m_iNPC_NUM && lpObjNPC->m_iCsNpcExistVal == pNpcData.m_iCsNpcExistVal && lpObjNPC->m_iCsNpcExistVal != NULL && lpObjNPC->Live == TRUE && lpObjNPC->Life > 0)
		{
			lpMsgBody[iCOUNT].iNpcNumber = pNpcData.m_iNPC_NUM;
			lpMsgBody[iCOUNT].iNpcIndex = pNpcData.m_iNPC_INDEX;
			lpMsgBody[iCOUNT].iNpcDfLevel = pNpcData.m_iNPC_DF_LEVEL;
			lpMsgBody[iCOUNT].iNpcRgLevel = pNpcData.m_iNPC_RG_LEVEL;
			lpMsgBody[iCOUNT].iNpcMaxHp = lpObjNPC->MaxLife;
			lpMsgBody[iCOUNT].iNpcHp = lpObjNPC->Life;
...
void CCastleSiege::SendNpcStateList(CGameObject &Obj, int iNpcType)
{
	...
	while( it != this->m_vtNpcData.end() )
	{
		...
		if( NpcData.m_iNPC_NUM == iNpcType && NpcData.m_bIN_USE == TRUE )
		{
			...
			if( gObjIsConnected(*lpObjNPC) )
			{
				if( lpObjNPC->Class == iNpcType && lpObjNPC->Live == 1 && lpObjNPC->Life > 0 && lpObjNPC->m_btCsNpcType != 0 && lpObjNPC->m_iCsNpcExistVal == NpcData.m_iCsNpcExistVal && lpObjNPC->m_iCsNpcExistVal != 0 ) //HermeX Fix
				{
					iLife = lpObjNPC->Life;
					iMaxLife = lpObjNPC->MaxLife;
					btNpcLive = 1;
				}
				else
				{
	...
	GIOCP.DataSend(Obj.m_Index, (BYTE*)&cNPC_LIST, (sizeof(PMSG_NPCDBLIST) * iNPC_COUNT)+sizeof(PMSG_ANS_NPCDBLIST));
}
...
void CCastleSiege::SendCsGateStateViewPort(int iGateIndex, BYTE btOperate)
{
	...
	for( int i = 0; i < MAX_VIEWPORT; i++ )
	{
		...
		if( tObjNum >= 0 )
		{
			if( getGameObject(tObjNum)->Type == OBJ_USER && gObjIsConnected(tObjNum) )
			{
...
int CCastleSiege::CreateCsGateLever(int iX, int iY)
{
	...
	if(iNPC_INDEX >= 0)
	{
		...
		getGameObject(iNPC_INDEX)->m_PosNum = -1;
		...
		getGameObject(iNPC_INDEX)->MaxLife = 10000.0f;
		getGameObject(iNPC_INDEX)->m_btCsNpcType = 2;
		getGameObject(iNPC_INDEX)->m_iCsGateLeverLinkIndex = -1;
...
{
	if(!gObjIsConnected(iLeverIndex) || getGameObject(iLeverIndex)->Class != 219)
	{
		return false;
	}

	if(!gObjIsConnected(iGateIndex) || getGameObject(iGateIndex)->Class != 277)
	{
		return false;
	}

	getGameObject(iLeverIndex)->m_iCsGateLeverLinkIndex = iGateIndex;
	getGameObject(iGateIndex)->m_iCsGateLeverLinkIndex = iLeverIndex;
	return true;
}

int CCastleSiege::CheckLeverAlive(int iLeverIndex)
{
	if(gObjIsConnected(iLeverIndex) == FALSE || getGameObject(iLeverIndex)->Class != 219 )
	{
		return false;
	}
...
void CCastleSiege::CheckCsDbNpcAlive()
{
	...
	if( this->m_vtNpcData.empty() == false )
	{
		for( std::vector<_CS_NPC_DATA>::iterator it = this->m_vtNpcData.begin(); it != this->m_vtNpcData.end(); it++ )
		{
			...
			{
				if( !gObjIsConnected(NpcData.m_iNPC_OBJINDEX) || getGameObject(NpcData.m_iNPC_OBJINDEX]->m_iCsNpcExistVal != NpcData.m_iCsNpcExistVal || getGameObject(NpcData.m_iNPC_OBJINDEX]->m_iCsNpcExistVal == 0 || (getGameObject(NpcData.m_iNPC_OBJINDEX)->Class != 277 && getGameObject(NpcData.m_iNPC_OBJINDEX)->Class != 283) || getGameObject(NpcData.m_iNPC_OBJINDEX)->Live == FALSE) //HermeX Fix
				{
...
void CCastleSiege::SetAllUserCsJoinSide()
{
	...
	for(int iUSER = g_ConfigRead.server.GetObjectStartUserIndex(); iUSER < g_ConfigRead.server.GetObjectMax(); iUSER++)
	{
		...
		if( it != this->m_mapCsTotalGuildInfo.end() )
		{
			getGameObject(iUSER)->m_btCsJoinSide = it->second.m_iCsGuildID;
			getGameObject(iUSER)->m_bCsGuildInvolved = (it->second.m_bGuildInvolved != 0);
		}

		this->NotifySelfCsJoinSide(iUSER);

		getGameObject(iUSER)->m_iAccumulatedCrownAccessTime = 0;
	}
...
void CCastleSiege::ResetAllUserCsJoinSide()
{
	...
	for(int iUSER = g_ConfigRead.server.GetObjectStartUserIndex(); iUSER < g_ConfigRead.server.GetObjectMax(); iUSER++)
	{
		...
		getGameObject(iUSER)->m_btCsJoinSide = 0;
		getGameObject(iUSER)->m_bCsGuildInvolved = 0;
		getGameObject(iUSER)->m_btLifeStoneCount = 0;

		this->NotifySelfCsJoinSide(iUSER);

		getGameObject(iUSER)->m_iAccumulatedCrownAccessTime = 0; //HermeX Fix 
	}
...
void CCastleSiege::NotifySelfCsJoinSide(CGameObject &Obj)
{
	...
	BYTE btCsJoinSide = Obj.m_btCsJoinSide; //loc4
	...
	{
		pMsg.btCsJoinSide = Obj.m_btCsJoinSide;
	}
	else
	{
...
BOOL CCastleSiege::CheckCastleOwnerUnionMember(CGameObject &Obj)
{
	...
	lpGuildInfo = Obj.m_PlayerData->lpGuild;
...
//		getGameObject(iUSER)->m_ViewSkillState &= 0xFFF3FFFF;
//		getGameObject(iUSER)->m_ViewSkillState &= 0xFFBFFFFF; //HermeX Fix
//		getGameObject(iUSER)->m_ViewSkillState &= 0xFF7FFFFF; //HermeX Fix
...
BOOL CCastleSiege::CheckMiddleWinnerGuild()
{
	...
	if( (strcmp(getGameObject(this->m_iCastleCrownAccessUser]->m_PlayerData->GuildName, "") == 0) || (strcmp(getGameObject(this->m_iCastleSwitchAccessUser1)->m_PlayerData->GuildName, "") == 0) || (strcmp(getGameObject(this->m_iCastleSwitchAccessUser2)->m_PlayerData->GuildName, "") == 0) )
	{
		return FALSE;
	}

	if( (getGameObject(this->m_iCastleCrownAccessUser]->m_btCsJoinSide >= 2 ) && (getGameObject(this->m_iCastleSwitchAccessUser1)->m_btCsJoinSide >= 2 ) && (getGameObject(this->m_iCastleSwitchAccessUser2)->m_btCsJoinSide >= 2 ) )
	{
		if( getGameObject(this->m_iCastleCrownAccessUser)->m_bCsGuildInvolved == 1 )
		{
			if( (getGameObject(this->m_iCastleCrownAccessUser]->m_btCsJoinSide == getGameObject(this->m_iCastleSwitchAccessUser1]->m_btCsJoinSide) && (getGameObject(this->m_iCastleCrownAccessUser)->m_btCsJoinSide == getGameObject(this->m_iCastleSwitchAccessUser2)->m_btCsJoinSide) )
			{
				if( ( ( GetTickCount() - this->m_dwCrownAccessTime ) + getGameObject(this->m_iCastleCrownAccessUser)->m_iAccumulatedCrownAccessTime ) >= 60000 ) //season4.5 changed
				{
					sLog->outBasic("[CastleSiege] Castle Winner Has been Changed (GUILD:%s)", getGameObject(this->m_iCastleCrownAccessUser)->m_PlayerData->GuildName);
					...
					this->NotifyAllUserCsProgState(1, getGameObject(this->m_iCastleCrownAccessUser)->m_PlayerData->GuildName);
...
BOOL CCastleSiege::CheckUnionGuildMaster(CGameObject &Obj)
{
	GUILD_INFO_STRUCT &lpGuildInfo;
...
int CCastleSiege::CheckOverlapCsMarks(CGameObject &Obj)
{
...
void CCastleSiege::NotifyCsSelfLeftTime(CGameObject &Obj)
{
	...
	while(it != this->m_vtNpcData.end())
	{
		...
		if(pNpcData.m_bNPC_DBSAVE == 0)
		{
			...
			if(iNPC_INDEX >= 0)
			{
				...
				getGameObject(iNPC_INDEX)->MaxLife = pNpcData.m_iNPC_MAXHP;
				...
				case 215:
					...
					break;
				case 221:
					getGameObject(iNPC_INDEX)->m_btCsNpcType = 3;
					break;
				case 222:
					getGameObject(iNPC_INDEX)->m_btCsNpcType = 2;
					break;
	...
	while(it != this->m_vtNpcData.end())
	{
		...
		if(pNpcData.m_iNPC_NUM == 283 && pNpcData.m_iNPC_LIVE == 0 )
		{
			...
			if(iNPC_INDEX >= 0)
			{
				...
				getGameObject(iNPC_INDEX)->Life = pNpcData.m_iNPC_HP;
				getGameObject(iNPC_INDEX)->MaxLife = pNpcData.m_iNPC_MAXHP;
				getGameObject(iNPC_INDEX)->m_btCsNpcType = 1;
				...
				it->m_btCsNpcExistVal1 = rand()%255+1;
				getGameObject(iNPC_INDEX)->m_btCsNpcExistVal1 = it->m_btCsNpcExistVal1;

				it->m_btCsNpcExistVal2 = rand()%256;
				getGameObject(iNPC_INDEX)->m_btCsNpcExistVal2 = it->m_btCsNpcExistVal2;

				it->m_btCsNpcExistVal3 = rand()%256;
				getGameObject(iNPC_INDEX)->m_btCsNpcExistVal3 = it->m_btCsNpcExistVal3;

				it->m_btCsNpcExistVal4 = rand()%256;
				getGameObject(iNPC_INDEX)->m_btCsNpcExistVal4 = it->m_btCsNpcExistVal4;
...
void CCastleSiege::ReSpawnAllUser()
{
	...
	for( int iUSER = g_ConfigRead.server.GetObjectStartUserIndex(); iUSER < g_ConfigRead.server.GetObjectMax(); iUSER++ )
	{
		...
		if( getGameObject(iUSER)->MapNumber != MAP_INDEX_CASTLESIEGE )
		{
			continue;
		}

		BYTE btJoinSide = getGameObject(iUSER)->m_btCsJoinSide;
		...
		if( bMoveOK )
		{
			getGameObject(iUSER)->m_State = 32;
			getGameObject(iUSER)->X = iTX;
			getGameObject(iUSER)->Y = iTY;
			getGameObject(iUSER)->TX = iTX;
			getGameObject(iUSER)->TX = iTY;
			getGameObject(iUSER)->MapNumber = MAP_INDEX_CASTLESIEGE;
			getGameObject(iUSER)->PathCount = 0;
			getGameObject(iUSER)->Teleport = 0;

			gObjClearViewport(getGameObject(iUSER));
			GSProtocol.GCTeleportSend(getGameObject(iUSER), -1, MAP_INDEX_CASTLESIEGE, getGameObject(iUSER)->X, getGameObject(iUSER)->Y, getGameObject(iUSER)->Dir);
			...
			getGameObject(iUSER)->RegenMapY = iTY;
...
void CCastleSiege::ReSpawnEnemyUser(int bRefreshOwnerUser)
{
	...
	for(int iUSER = g_ConfigRead.server.GetObjectStartUserIndex(); iUSER < g_ConfigRead.server.GetObjectMax(); iUSER++)
	{
		...
		BYTE btCsJoinSide = getGameObject(iUSER)->m_btCsJoinSide;
		...
		case 1:
			if(bRefreshOwnerUser != FALSE)
			{
				...
				GSProtocol.GCTeleportSend(getGameObject(iUSER),-1,getGameObject(iUSER)->MapNumber,getGameObject(iUSER)->X,getGameObject(iUSER)->Y,getGameObject(iUSER)->Dir); //season 2.5 changed
...
void CCastleSiege::ClearCastleTowerBarrier()
{
	for( int iIDX = 0; iIDX < g_ConfigRead.server.GetObjectMaxMonster(); iIDX++ )
	{
		...
		if( getGameObject(iIDX)->Class == 215 )
		{
			gObjDel(iIDX);
		}
	}
...
void CCastleSiege::SetAllCastleGateState(BOOL bOpenType)
{
	...
	while(it != m_vtNpcData.end())
	{
		...
		if(pNpcData.m_iNPC_NUM == 277 && gObjIsConnected(pNpcData.m_iNPC_OBJINDEX) && getGameObject(pNpcData.m_iNPC_OBJINDEX)->Class == 277)
		{
			switch(bOpenType)
			{
			case 0:
...
void CCastleSiege::AddMiniMapDataReqUser(CGameObject &Obj)
{
...
void CCastleSiege::DelMiniMapDataReqUser(CGameObject &Obj)
{
	EnterCriticalSection(&this->m_critCsMiniMap);
...
void CCastleSiege::OperateMiniMapWork()
{
	...
	if( this->m_vtNpcData.empty() == false )
	{
		for( std::vector<_CS_NPC_DATA>::iterator it = this->m_vtNpcData.begin(); it != this->m_vtNpcData.end(); it++ )
		{
			...
			if( (NpcData.m_iNPC_NUM == 277 || NpcData.m_iNPC_NUM == 283) && NpcData.m_iNPC_OBJINDEX != OBJ_EMPTY )
			{
				if( !gObjIsConnected(NpcData.m_iNPC_OBJINDEX) || getGameObject(NpcData.m_iNPC_OBJINDEX]->m_iCsNpcExistVal != NpcData.m_iCsNpcExistVal || (getGameObject(NpcData.m_iNPC_OBJINDEX)->Class != 277 && getGameObject(NpcData.m_iNPC_OBJINDEX)->Class != 283) )
				{
					BYTE btNpcType = 0;
					...
					lpMsgSendBody2[lpMsgSend2->iCount].btX = getGameObject(NpcData.m_iNPC_OBJINDEX)->X;
					lpMsgSendBody2[lpMsgSend2->iCount].btY = getGameObject(NpcData.m_iNPC_OBJINDEX)->Y;
	...
	for( int iIDX = g_ConfigRead.server.GetObjectStartUserIndex(); iIDX < g_ConfigRead.server.GetObjectMax(); iIDX++ )
	{
		...
		if( getGameObject(iIDX)->m_btCsJoinSide == 0 )
		{
			continue;
		}

		std::map<int, _CS_MINIMAP_DATA>::iterator it3(this->m_mapMiniMapData.find(getGameObject(iIDX)->m_btCsJoinSide));
		...
		if( it3 != this->m_mapMiniMapData.end() )
		{
			...
			it3->second.m_stMiniMapPoint[it3->second.m_iMiniMapPointCount].btX = getGameObject(iIDX)->X;
			it3->second.m_stMiniMapPoint[it3->second.m_iMiniMapPointCount].btY = getGameObject(iIDX)->Y;
...
void CCastleSiege::NotifyCrownState(BYTE btState)
{
	...
	if( getGameObject(m_iCrownIndex)->Class != 216 )
	{
		sLog->outError(  "[CastleSiege] CCastleSiege::NotifyCrownState() - m_iCrownIndex isn't Valid");
		return;
	}
