...
void CCrywolfAltar::SetAltarValidContract(int iAltarObjIndex, int iClass)
{
	...
	if ( AltarInfo->m_dwValidContractTime < GetTickCount() )
	{
		...
		UTIL.SendCrywolfUserAnyMsg(2, Lang.GetText(0,244), getGameObject(AltarInfo->m_iUserIndex)->Name, iAltarNumber);

		sLog->outBasic("[Crywolf][Altar op.]  [%s][%s] Set Valid Contract Altar[%d]",
			getGameObject(AltarInfo->m_iUserIndex]->AccountID, getGameObject(AltarInfo->m_iUserIndex)->Name, iAltarNumber);
...
BOOL CCrywolfAltar::SetAltarUserIndex(int iAltarObjIndex, int iClass, CGameObject &Obj)
{
	...
	sLog->outBasic("[Crywolf][Altar Op.] [%s][%s] Attempt to contract Altar[%d]",
		getGameObject(AltarInfo->m_iUserIndex]->AccountID, getGameObject(AltarInfo->m_iUserIndex)->Name,
		iAltarNumber);
...
void CCrywolfAltar::SetAllAltarObjectIndex()
{
	for ( int i=0;i<g_ConfigRead.server.GetObjectMaxMonster();i++)
	{
		...
		if ( CRYWOLF_ALTAR_CLASS_RANGE(getGameObject(i)->Class ) != FALSE )
			g_CrywolfNPC_Altar.SetAltarObjIndex(getGameObject(i)->Class, i);
	}
}
