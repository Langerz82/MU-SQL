...
private:
	...
	BOOL CanWeMoveForward2(int x, int y) // line : 46
	{
		...
		if ( DG_MAP_RANGE(this->m_nMapNumber) == TRUE )
		{
			if ( this->Map[which] == 1 || this->Map[which] == 4 )
			{
				this->HitMap[which] = 1;
				return false;
			}
		...
		else if ( this->Map[which] >= 1 || this->HitMap[which] != 0 )
		{
			this->HitMap[which] = 1;
			return false;
		}
...
public:
	...
	bool FindPath(int startx, int starty, int endx,  int endy, bool ErrorCheck) // line : 257
	{
		...
		int sizeToNull = this->Width * this->Height;
		memset(this->HitMap, 0, sizeToNull);
		First = true;

		while ( (startx != endx || starty != endy) && (this->m_nMapNumber < 69 || this->m_nMapNumber > 72 || this->NumPath < 8 ) )
		{
	...
	void SetMapDimensions(int w, int h, LPBYTE map, int nMapNumber) // line : 355
	{
		this->Width = w;
		this->Height = h;
