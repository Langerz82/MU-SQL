...
BOOL CPentagramSystem::PentagramJewel_IN(int aIndex, int iPentagramItemPos, int iJewelItemPos)
{
	...
	{
		g_Log.Add("[PentagramJewel_IN] PentagramItem Socket is Not Empty [%s][%s]",lpObj->AccountID, lpObj->Name);
		return FALSE;
	}
...
void CPentagramSystem::LoadOptionMaps(const char* File)
{
	...
	{
		g_Log.MsgBox("[IGC_PentagramDropMaps] File %s not found!", File);
		return;
	}
...
BOOL CPentagramSystem::ElementDrop(LPOBJ lpObj, LPOBJ lpTargetObj)
{
