...
void gObjCharZeroSet(int aIndex)
{
	...
	if (lpObj->Type == OBJ_USER && (lpObj->m_PlayerData->ISBOT == false && lpObj->m_PlayerData->bt_BotType != 1))
	{
		...
		if (!pCEvoMonInfo)
		{
			g_Log.Add("[EvoMon][Error] pCEvoMonInfo is NULL [%s][%s] [%s, %d]", lpObj->AccountID, lpObj->Name, __FILE__, __LINE__);
		}
...
bool gObjLevelUp(LPOBJ lpObj, UINT64 addexp, int iMonsterType, const char * szEventType)
{
	...
	//g_Log.Add("[%s] Experience : Map[%s]-(%d,%d) [%s][%s](%d) Experience: %I64d + %I64d  MonsterClass : %d",szEventType, Lang.GetMap(0, lpObj->MapNumber),lpObj->X,lpObj->Y,lpObj->AccountID,lpObj->Name,lpObj->Level,lpObj->m_PlayerData->Experience - addexp,addexp,iMonsterType);
	...
	if (g_ConfigRead.data.reset.iBlockLevelUpPointAfterResets == -1 || lpObj->m_PlayerData->m_iResets < g_ConfigRead.data.reset.iBlockLevelUpPointAfterResets)
	{
		...
		if (lpObj->m_PlayerData->PlusStatQuestClear != false)
		{
			...
			//g_Log.Add("[%s][%s] LevelUp PlusStatQuest Clear AddStat %d",lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->LevelUpPoint);
	...
	if (lpObj->Class == CLASS_ELF)
	{
		if (lpObj->Level >= g_SkillAdditionInfo.GetInfinityArrowUseLevel() && lpObj->m_PlayerData->ChangeUP >= 1)
		{
			...
			if (iAddSkillPosition >= 0)
			{
				...
				//g_Log.Add("[%s][%s] Add Infinity Arrow Skill (Character Level : %d)(ChangeUp: %d)",	lpObj->AccountID,lpObj->Name,lpObj->Level,lpObj->m_PlayerData->ChangeUP);
			}
			else
			{
				//g_Log.Add("[%s][%s] Fail - Add Infinity Arrow Skill (Character Level : %d)(ChangeUp: %d)",lpObj->AccountID,lpObj->Name,lpObj->Level,lpObj->m_PlayerData->ChangeUP);
			}
		}
	}
...
BOOL gObjLevelUpPointAdd(BYTE type, LPOBJ lpObj, int aIndex)	// type : [1:Strength][2:Agility][3:Vitality][4:Energy][5:Leadership]
{
	if (lpObj->Type != OBJ_USER)
	{
...
BOOL gObjInventoryEquipment(LPOBJ lpObj)
{
	...
	if (lpObj->m_btInvenPetPos >= INVETORY_WEAR_SIZE && lpObj->m_btInvenPetPos < MAIN_INVENTORY_SIZE)
	{
		...
		//g_Log.Add("[CRenewal][InvenPet][InvenPetBuff][%s][%s] PetIndex:[%d], Pos:[%d]",lpObj->AccountID, lpObj->Name, lpObj->pInventory[lpObj->m_btInvenPetPos].m_Type, lpObj->m_btInvenPetPos);

		if (lpEffect == NULL)
		{
			//g_Log.Add("[CRenewal][InvenPet][InvenPetBuff][%s][%s] PetIndex:[%d], Pos:[%d] [Fail] lpBuffData is Null",lpObj->AccountID, lpObj->Name, lpObj->pInventory[lpObj->m_btInvenPetPos].m_Type, lpObj->m_btInvenPetPos);
			return FALSE;
		}
		...
		if (lpObj->pInventory[lpObj->m_btInvenPetPos].m_JewelOfHarmonyOption == 1)
		{
			...
			//g_Log.Add("[CRenewal][InvenPet][ReqInvenPetBuff][%s][%s] PetIndex:[%d], Pos:[%d] [Success] Register InvenPet Buff",	lpObj->AccountID, lpObj->Name, lpObj->pInventory[lpObj->m_btInvenPetPos].m_Type, lpObj->m_btInvenPetPos);
		...
		else
		{
			...
			//g_Log.Add("[CRenewal][InvenPet][RemoveInvenPetBuff][%s][%s] PetIndex:[%d], Pos:[%d] [Success] Remove InvenPet Buff",lpObj->AccountID, lpObj->Name, lpObj->pInventory[lpObj->m_btInvenPetPos].m_Type, lpObj->m_btInvenPetPos);
		...
		if (titem->IsItem() == 1)
		{
			...
			if (t_dur < max_count)
			{
				...
				if (titem->m_Type == ITEMGET(12, 144) && max_count <= titem->m_Durability && titem->m_BonusSocketOption == sitem->m_BonusSocketOption)
				{
					...
					ItemSerialCreateSend(aIndex, 235, gObj[aIndex].X, gObj[aIndex].Y, ItemGetNumberMake(12, 145), sitem->m_Level, 0, 0, 0, 0, aIndex, 0, 0, 0, 0, sitem->m_BonusSocketOption);
				}
				...
				if (titem->m_Type == ITEMGET(14, 290) && max_count <= titem->m_Durability)
				{
					...
					ItemSerialCreateSend(aIndex, 235, gObj[aIndex].X, gObj[aIndex].Y, ItemGetNumberMake(14, 293), sitem->m_Level, 0, 0, 0, 0, aIndex, 0, 0, 0, 0, 0);
				}
			...
			if (g_ConfigRead.MovingSpeedCheck == TRUE)
			{
				...
				if (lpObj->m_PlayerData->MovingIgnore == 0 && lpObj->m_PlayerData->MovingDistance >= g_ConfigRead.MovingSpeedDistance && lpObj->MapNumber != MAP_INDEX_HATCHERY)
				{
					if (g_ConfigRead.MovingSpeedLog != 0)
						g_Log.Add("[%s][%s] Move Speed Hack Detected [%d]", lpObj->AccountID, lpObj->Name, lpObj->m_PlayerData->MovingDistance);
	...
	else
	{
		...
		if ((lpObj->m_RecallMon >= 0 && (lpObj->Class < 678 || lpObj->Class > 680)) || g_EvoMonMng.IsEvolutedEvoMon(lpObj->m_Index) == true || lpObj->Class == 690)
		{
			...
			if (lpObj->Class == 707) // Dark Wizard Phantom
			{
				...
				if (ObjectMaxRange(iOwnerIndex) == true && gObj[iOwnerIndex].Type == OBJ_USER)
				{
					memcpy(pCallMonsterViewportCreate.CharSet, gObj[iOwnerIndex].CharSet, 18);
				}
			}
			...
			if (ObjectMaxRange(iOwnerIndex) == true && gObj[iOwnerIndex].Type == OBJ_USER)
			{
				memcpy(pCallMonsterViewportCreate.Id, gObj[iOwnerIndex].Name, sizeof(pCallMonsterViewportCreate.Id));
			...
			if (ObjectMaxRange(iOwnerIndex) == true && gObj[iOwnerIndex].Type == OBJ_USER)
			{
				memcpy(pCallMonsterViewportCreate.Id, gObj[iOwnerIndex].Name, MAX_ACCOUNT_LEN);
			}
...
BOOL gObjMoveGate(int aIndex, int gt)
{
	...
	{
		userlevel = gObj[aIndex].Level + gObj[aIndex].m_PlayerData->MasterLevel;
	}
	else
	{
...
int gObjCheckOverlapEventItemUsingDur(int iUserIndex, int iMaxOverlapped, int iItemType, int iItemLevel)
{
	...
	{
		if (gObj[iUserIndex].pEventInventory[x].IsItem() == 1
			&& gObj[iUserIndex].pEventInventory[x].m_Type == (short)iItemType
			&& gObj[iUserIndex].pEventInventory[x].m_Level == (short)iItemLevel)
		{
			int iITEM_DUR = gObj[iUserIndex].pEventInventory[x].m_Durability;
...
int gObjOverlapEventItemUsingDur(class CItem* lpItem, int iMapNumber, int iItemNumber, int iUserIndex, int iMaxOverlapped, int iItemType, int iItemLevel)
{
	for (int iLoop = 0; iLoop < EVENT_INVENTORY_SIZE; iLoop++)
	{
		...
		{
			int iItemDur = gObj[iUserIndex].pEventInventory[iInventoryIndex].m_Durability + lpItem->m_Durability;
			...
			{
				lpItem->m_Durability = iItemDur - iMaxOverlapped;
				gObj[iUserIndex].pEventInventory[iInventoryIndex].m_Durability = iMaxOverlapped;

				GSProtocol.GCEventItemDurSend(iUserIndex, iInventoryIndex, gObj[iUserIndex].pEventInventory[iInventoryIndex].m_Durability);
			}
		}
