...
{
	this->Init();
	this->Read(g_ConfigRead.GetPath(FILE_CUSTOM_USERDIEITEMDROP));
}
// -------------------------------------------------------------------------------
...
void UserDieItemDrop::Read(LPSTR File)
{
	...
	{
		g_Log.MsgBox("[UserDieItemDrop] File %s not found!", File);
		return;
	}
	// ----
	...
	for (int i = 0; i < this->m_RollMaxCount; i++)
	{
		for (int n = 0; n < INVENTORY_MAP_SIZE; n++)
		{
				...
				&& !g_kJewelOfHarmonySystem.IsStrengthenByJewelOfHarmony(&lpUser->pInventory[Slot]))
			{
				...
				{
					g_Log.Add("[UserDieItemDrop] [%s][%s] Item has been droped (Slot: %d)",	lpUser->AccountID, lpUser->Name);
					break;
				}
			}
		}
