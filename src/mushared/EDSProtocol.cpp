...
void GDReqAllowJoinGuildMatching(int nUserIndex, int nAllowType, char *szName, char *szGuildName)
{
	...
	if (nAllowType == 1)
	{
		...
		else if (IT_MAP_RANGE(gObj[nUserIndex].MapNumber) == TRUE || ITL_MAP_RANGE(gObj[nUserIndex].MapNumber) == TRUE)
		{
			MsgOutput(nUserIndex, Lang.GetText(0, 569));
			bError = TRUE;
		}

		else if (g_NewPVP.IsDuel(gObj[nUserIndex]) == TRUE)
		{
			GSProtocol.GCServerMsgStringSend(Lang.GetText(0, 570), nUserIndex, 1);
			bError = TRUE;
		}

		else if (g_NewPVP.IsObserver(gObj[nUserIndex]) == TRUE)
		{
			GSProtocol.GCServerMsgStringSend(Lang.GetText(0, 571), nUserIndex, 1);
			bError = TRUE;
		}

		else if (iArcaBattleState > 2 && iArcaBattleState < 9)
		{
			GSProtocol.GCResultSend(nUserIndex, 0x51, 0xA4);
			bError = TRUE;
		}

		else if (gObj[nUserIndex].m_IfState.use > 0)
		{
			GSProtocol.GCResultSend(nUserIndex, 0x51, 0x06);
			bError = TRUE;
		}

		else if ((gObj[nUserIndex].m_Option & 1) != 1)
		{
			GSProtocol.GCResultSend(nUserIndex, 0x51, 0x00);
			bError = TRUE;
		}

		else if (gObj[nUserIndex].m_PlayerData->GuildNumber <= 0)
		{
			GSProtocol.GCResultSend(nUserIndex, 0x51, 0x04);
			bError = TRUE;
		}

		else if (gObj[nUserIndex].m_PlayerData->lpGuild == NULL)
		{
			GSProtocol.GCResultSend(nUserIndex, 0x51, 0x04);
			bError = TRUE;
		}

		else if (gObj[nUserIndex].m_PlayerData->lpGuild && strcmp(gObj[nUserIndex].m_PlayerData->lpGuild->Names[0], gObj[nUserIndex].Name))
		{
			GSProtocol.GCResultSend(nUserIndex, 0x51, 0x04);
			bError = TRUE;
		}

		else if (gObj[nUserIndex].m_PlayerData->lpGuild && gObj[nUserIndex].m_PlayerData->lpGuild->WarState)
		{
			GSProtocol.GCResultSend(nUserIndex, 0x51, 0x06);
			bError = TRUE;
		}
		...
		else
		{
			...
			if (gObj[nUserIndex].m_PlayerData->lpGuild && gObj[nUserIndex].m_PlayerData->lpGuild->TotalCount >= MaxGuildMember)
			{
