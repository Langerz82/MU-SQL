...
void CChaosCastle::CheckSync(int iChaosCastleIndex)
{
	...
	//g_Log.Add("[Chaos Castle] (%d) Sync Start Time. [%d] min remain (START HOUR:%d, MIN:%d)",iChaosCastleIndex+1, this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC / 60000,
...
void CChaosCastle::ProcState_Playing(int iChaosCastleIndex)
{
	...
	if ( iTICK_MSEC >= 1000 )
	{
			...
			this->m_stChaosCastleData[iChaosCastleIndex].m_bCC_PLAY_START == false )
		{
			...
			//g_Log.Add("[Chaos Castle] (%d) Chaos Castle Quest Start (UserCount:%d)",iChaosCastleIndex+1, this->GetCurPlayUser(iChaosCastleIndex));
		}
			 ...
			 this->m_stChaosCastleData[iChaosCastleIndex].m_bCC_CAN_ENTER == false )
		{
			...
			{
				this->SetState(iChaosCastleIndex, CC_STATE_CLOSED);
				//g_Log.Add("[Chaos Castle] (%d) Chaos Castle Quest Closed - No User",iChaosCastleIndex+1);
		...
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_bCC_PLAY_START == true )
		{
			...
			if ( bPlayEnded == TRUE )
			{
				...
				{
					//g_Log.Add("[Chaos Castle] (%d) Has No Winner : Monster Left (%d)",	iChaosCastleIndex+1, this->m_stChaosCastleData[iChaosCastleIndex].m_lCC_CURRENT_MONSTER_COUNT);
				}
	...
	if ( this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC <= 0 )
	{
		//g_Log.Add("[Chaos Castle] (%d) is Over : TIME-OUT (Left User:%d, Monster:%d)",iChaosCastleIndex+1, this->GetCurPlayUser(iChaosCastleIndex), this->GetMonsterListCount(iChaosCastleIndex));
...
BOOL CChaosCastle::LevelUp(int iUserIndex, int iAddExp)
{
	...
	//g_Log.Add("[Chaos Castle] Experience : [%s][%s](%d) Experience: %d + %d",gObj[iUserIndex].AccountID,	gObj[iUserIndex].Name,
				 ...
				 gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == this->GetChaosCastleMapNumber(iChaosCastleIndex) )
			{
				if ( gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Life > 0.0 )
				{
					...
					if ( btMapAttr == 8 )
					{
						...
						//g_Log.Add("[Chaos Castle] (%d) [%s][%s] User Dead In Chaos Castle : Fall from Castle (X:%d, Y:%d)",	iChaosCastleIndex+1, gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].AccountID,
	...
	for ( int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1 )
		{
				 ...
				 gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == this->GetChaosCastleMapNumber(iChaosCastleIndex) )
			{
				...
				//g_Log.Add("[Chaos Castle] (%d) [%s][%s] Payback to User ChaosCastle Money (FIRST:%d, PAYBACK:%d, FINAL:%d)",	iChaosCastleIndex+1, gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].AccountID,
...
void CChaosCastle::RewardUserEXP(int iChaosCastleIndex, int iChaosCastleSubIndex, BOOL bWinner)
{
	...
	if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex != -1 )
	{
		if ( gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex) ==TRUE )
		{
			...
			//g_Log.Add("[Chaos Castle] (%d) [%s][%s] Reward User EXP (USER_KILL:%d, MON_KILL:%d, TOT_EXP:%d, TOT_REWARD_EXP:%d)",iChaosCastleIndex+1, gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex].AccountID,
