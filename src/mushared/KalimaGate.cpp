		...
		if ( gObj[iIndex].pInventory[x].IsItem() == TRUE )
		{
			if ( gObj[iIndex].pInventory[x].m_Type == ITEMGET(14, 29) )
			{
				if ( gObj[iIndex].pInventory[x].m_Level == btLevel )
				{
					...
					if (iITEM_DUR >= 0 && iITEM_DUR <= IsOverlapItem(gObj[iIndex].pInventory[x].m_Type))
					{
						return x;
					}
				}
...
{
	if ( gObj[iIndex].Class == CLASS_MAGUMSA || gObj[iIndex].Class == CLASS_DARKLORD || gObj[iIndex].Class == CLASS_RAGEFIGHTER )	// DarkLord and MagicGLadiaro
	{
...
BOOL CKalimaGate::DeleteKalimaGate(int iKalimaGateIndex, int iCallOwnerIndex)
{
	...
	__try
	{
		...
		GSProtocol.GCDiePlayerSend( &gObj[iKalimaGateIndex], iKalimaGateIndex, 0 , 0);
