...
void CEventMonSetBase::RegenMonster(int iIndex)
{
	...
	//g_Log.Add("[Event Spawn] Regen Monster (%d)(%d) (%d)(%d)(%d)",iIndex, lpObj->Class, lpObj->MapNumber, lpObj->X, lpObj->Y);
}
...
void CEventMonSetBase::DeleteAllMonsters()
{
	...
	{
		//g_Log.Add("[Event Spawn] Delete Monster (%d)(%d) (%d)(%d)(%d)", It->first,	gObj[It->first].Class, gObj[It->first].MapNumber, gObj[It->first].X, gObj[It->first].Y);
