...
BOOL wsJoinServerCli::Connect(LPSTR ip_addr, WORD port, DWORD WinMsgNum)
{
	...
	if ( this->m_hWnd  == 0 )
	{
		g_Log.Add("Connect Error %s %d", __FILE__, __LINE__);
		return FALSE;
	}
	...
	if ( addr.sin_addr.S_un.S_addr == -1 )
	{
		...
		if ( host == 0 )
		{
			g_Log.Add("server name not found [%s]", ip_addr);
			return FALSE;
		}
	...
	if ( nResult == -1 )
	{
		...
		g_Log.Add("Client WSAAsyncSelect error %d", WSAGetLastError());
		return FALSE;
	}
	this->m_SendBufLen=0;
...
BOOL wsJoinServerCli::DataSend(PCHAR buf, int len)
{
	...
	while ( true )
	{
		...
		if ( nResult == -1)
		{
			...
			{
				g_Log.Add("error-L3 : %d (%s %d)", WSAGetLastError(), __FILE__, __LINE__);
				*sendbuflen=0;
				this->Close(this->m_socket );
				return FALSE;
			}
			...
			{
				g_Log.AddC(TColor::Red, "error-L3 : buffer error closed %d, %d", (*sendbuflen+nLeft), 819200);
				*sendbuflen = 0;
				this->Close(this->m_socket);
				return FALSE;
			}
			...
			if ( nDx >= 0 )
			{
				...
				g_Log.Add("error-L3 : Copy1 : nLeft:%d len:%d nDx:%d", nLeft, this->m_SendBufLen , nDx);
				return TRUE;
			}
		}
		else if ( nResult == 0 )
		{
			g_Log.Add("error-L3 : send()  result is zero", WSAGetLastError());
			break;
		}
...
BOOL wsJoinServerCli::FDWRITE_MsgDataSend()
{
	...
	while (*sendbuflen>0)
	{
		...
		if (nResult == -1)
		{
			if ( WSAGetLastError() != WSAEWOULDBLOCK)
			{
				g_Log.Add("FD_WRITE send() ¿¡·¯ %d %d", WSAGetLastError(), *sendbuflen);
				this->Close();
				return FALSE;
			}
			else
			{
				g_Log.Add("FD_WRITE send() WSAEWOULDBLOCK : %d", WSAGetLastError());
				break;
			}
		}
		if (nResult <= 0)
		{
			g_Log.Add("send() result is zero %d", WSAGetLastError());
			break;
		}
		g_Log.Add("error-L3 : nDx %d m_SendBufLen %d", nDx, this->m_SendBufLen );
...
int wsJoinServerCli::DataRecv()
{
	...
	if ( nResult == 0 )
	{
		g_Log.Add("error-L3 : closed %d - buff:%d(%d) ",
	...
	else if (nResult == SOCKET_ERROR )
	{
		...
		g_Log.Add("error-L3 : recv error %d", WSAGetLastError() );
		return 1;
	}
	...
	while ( true )
	{
		...
		if ( this->m_UseAuthProtocol == true )
		{
			if( this->m_bUseRSA == true && recvbuf[lOfs] != 0xF0 ) // like this
			{
				...
				if ( PrvKey == NULL )
				{
					g_Log.AddC(TColor::Red,  "[PACKET-ENCODE] ERROR: PrvKey Pointer is NULL");
					break;
				}
				...
				if ( size <= 0 )
				{
					g_Log.Add("error-L2 : size %d", size);
					return 0;
				}
		...
		else
		{
			...
			{
				g_Log.Add("error-L2 : header error (%s %d)lOfs:%d, size:%d", __FILE__, __LINE__, lOfs, *recvbuflen);
				*recvbuflen = 0;
				return 0;
			}
