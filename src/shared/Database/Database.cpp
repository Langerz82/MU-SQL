...
bool Database::CheckDatabaseVersion(DatabaseTypes database)
{
    ...
    { 
        ...
        sLog.outErrorDb();
        sLog.outErrorDb("  [A] You have database Version: MuMySQLServer can not verify your database version or its existence!");
        sLog.outErrorDb();
    ...
    if (version != dbversion.expected_version || structure != dbversion.expected_structure)
    {
        ...
        sLog.outErrorDb("                    Description: %s", dbversion.description.c_str());
        sLog.outErrorDb();
        sLog.outErrorDb("You must apply all updates after [A] to [B] to use MuMySQLServer with this database.");
