...
uint32 MySQLConnection::Open()
{
    ...
    if (!mysqlInit)
    {
        sLog->outBasic("Could not initialize Mysql connection to database `%s`", m_connectionInfo.database.c_str());
    ...
    if (m_Mysql)
    {
        ...
        sLog->outBasic("Connected to MySQL database at %s", m_connectionInfo.host.c_str());
...
bool MySQLConnection::Execute(char const* sql)
{
...

    {
        ...
        if (mysql_query(m_Mysql, sql))
        {
            ...
            MUSQL_LOG_DEBUG("sql.sql", "[%u ms] SQL: %s", WorldTimer::getMSTimeDiff(_s, WorldTimer::getMSTime()), sql);
    }
...
bool MySQLConnection::Execute(PreparedStatement* stmt)
{
    ...
    MUSQL_LOG_DEBUG("sql.sql", "[%u ms] SQL(p): %s", WorldTimer::getMSTimeDiff(_s, WorldTimer::getMSTime()), m_mStmt->getQueryString().c_str());
...
MySQLPreparedStatement* MySQLConnection::GetPreparedStatement(uint32 index)
{
    ...
    if (!ret)
        sLog->outBasic("Could not fetch prepared statement %u on database `%s`, connection type: %s.",
...
void MySQLConnection::PrepareStatement(uint32 index, std::string const& sql, ConnectionFlags flags)
{
    ...
    if (!stmt)
    {
        sLog->outBasic("In mysql_stmt_init() id: %u, sql: \"%s\"", index, sql.c_str());
        sLog->outBasic("%s", mysql_error(m_Mysql));
        m_prepareError = true;
    }
    else
    {
        ...
        {
            sLog->outBasic("In mysql_stmt_prepare() id: %u, sql: \"%s\"", index, sql.c_str());
            sLog->outBasic("%s", mysql_stmt_error(stmt));
...
bool MySQLConnection::_HandleMySQLErrno(uint32 errNo, uint8 attempts /*= 5*/)
{
    switch (errNo)
    {
        ...
        case CR_SERVER_LOST_EXTENDED:
        {
            if (m_Mysql)
            {
                sLog->outBasic("Lost the connection to the MySQL server!");
        ...
        case ER_NO_SUCH_TABLE:
            sLog->outBasic("Your database structure is not up to date. Please make sure you've executed all queries in the sql/updates folders.");
            std::this_thread::sleep_for(std::chrono::seconds(10));
            std::abort();
            return false;
        case ER_PARSE_ERROR:
            sLog->outBasic("Error while parsing SQL. Core fix required.");
            std::this_thread::sleep_for(std::chrono::seconds(10));
            std::abort();
            return false;
        default:
            sLog->outBasic("Unhandled MySQL errno %u. Unexpected behaviour possible.", errNo);
            return false;
    }
}
