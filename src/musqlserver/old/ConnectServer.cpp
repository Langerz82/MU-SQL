                     ...
                     int       nCmdShow)
{
	...
	GetPrivateProfileString("AutoUpdate", "Version", "1.06.06", g_ClientVersion, 9, ".\\ConnectServer.ini");
	GetPrivateProfileString("AutoUpdate", "VersionFile", "version.dat", g_VersionFile, 20, ".\\ConnectServer.ini");
	GetPrivateProfileString("AutoUpdate", "HostURL", "mu.trong.live", g_HostURL, 100, ".\\ConnectServer.ini");
	GetPrivateProfileString("AutoUpdate", "FTPLogin", "mu0nline", g_FTPLogin, 20, ".\\ConnectServer.ini");
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case ID_RELOAD_AUTOUPDATE:
			...
			GetPrivateProfileStringA("AutoUpdate", "HostURL", "mu.trong.live", g_HostURL, 100, ".\\ConnectServer.ini");
	...
	case WM_DESTROY:
		...
		break;
	case WM_CLOSE:
			if ( MessageBox(NULL, "Are you sure to Close? -(o_0)'--", "Confirmation", MB_YESNO|MB_ICONQUESTION|MB_TOPMOST) == IDYES )
				DestroyWindow(hWnd);
			break;
	default:
		return DefWindowProc(hWnd, message, wParam, lParam);
