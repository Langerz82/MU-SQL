                     ...
                     int       nCmdShow)
{
	...
	{
		DialogBox(hInstance, (LPCTSTR)IDD_DIALOGBAR, NULL, (DLGPROC)AuthWindow);
		return 0;
	}
...
bool IniteDataServer()
{
	...
	if (g_DSMode == TRUE)
	{
		wsprintf(szText, "[%s] DataServer [DS: %d][MD5: %d][MainDB: %s][AccountDB: %s][EventsDB: %s][RankingDB: %s] [Season 12]", DATASERVER_VERSION, g_DataServerListPort, g_PwEncrypt, g_MuOnlineDNS, g_MeMuOnlineDNS, g_EventServerDNS, g_RankingServerDNS);
	}
	else if (!g_UseDataServer && !g_UseExDataServer && g_UseJoinServer == TRUE && !g_DSMode)
	{
		wsprintf(szText, "[%s] JoinServer [JS: %d][MD5: %d][AccountDB: %s][Season 12]", DATASERVER_VERSION, g_JoinServerListPort, g_PwEncrypt, g_MeMuOnlineDNS);
	}
	else if (!g_UseDataServer && g_UseExDataServer == TRUE && !g_UseJoinServer && !g_DSMode)
	{
		wsprintf(szText, "[%s] ExDataServer [ExDS: %d][MD5: %d][MainDB: %s][AccountDB: %s][EventsDB: %s][RankingDB: %s] [Season 12]", DATASERVER_VERSION, g_ExDataServerListPort, g_PwEncrypt, g_MuOnlineDNS, g_MeMuOnlineDNS, g_EventServerDNS, g_RankingServerDNS);
	}
	else
	{
		wsprintf(szText, "[%s] IGC.DataServer [DS: %d][ExDS: %d][JS: %d][MD5: %d][MainDB: %s][AccountDB: %s][EventsDB: %s][RankingDB: %s] [Season 12]", DATASERVER_VERSION, g_DataServerListPort, g_ExDataServerListPort, g_JoinServerListPort, g_PwEncrypt, g_MuOnlineDNS, g_MeMuOnlineDNS, g_EventServerDNS, g_RankingServerDNS);
	}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message) 
	{
		...
		case WM_DESTROY:
			...
			break;
		case WM_CLOSE:
			if ( MessageBox(NULL, "Are you sure to Close? -'(0_o)--", "Confirmation", MB_YESNO|MB_ICONQUESTION|MB_TOPMOST) == IDYES )
				DestroyWindow(hWnd);
			break;
		default:
			return DefWindowProc(hWnd, message, wParam, lParam);
...
LRESULT CALLBACK AuthWindow(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        SetDlgItemText(hDlg, IDC_EDIT1, HWID);
        return TRUE;
