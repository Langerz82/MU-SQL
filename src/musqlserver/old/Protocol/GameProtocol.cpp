...
void GameProtocol::GCItemObjectInventoryPutSend(CGameObject &lpObj, BYTE result, BYTE iteminfo1, BYTE iteminfo2)
{
...
void GameProtocol::GCItemObjectMoveResultSend(CGameObject &lpObj, BYTE result, BYTE pos, LPBYTE const ItemInfo)
{
...
void GameProtocol::CGInventoryItemMove(PMSG_INVENTORYITEMMOVE * lpMsg, CGameObject &lpObj)
{
	...
	{
		this->GCItemObjectMoveResultSend(aIndex, -1, 0, (LPBYTE)&ItemInfo);
		...
		(lpMsg->sFlag == 20 && lpMsg->tFlag == 20))
	{
		...
		this->GCItemObjectMoveResultSend(aIndex, result, target, (LPBYTE)&ItemInfo);
		if (DurSSend != FALSE)
	...
	if (lpMsg->sFlag == 22 && lpMsg->tFlag == 22)
	{
		...
		GCItemObjectMoveResultSend(aIndex, result, target, (LPBYTE)ItemInfo);
	}
	...
	if (lpMsg->sFlag == 1 && lpMsg->tFlag == 0)
	{
		...
		if (result == 0xFF)
		{
			this->GCItemObjectMoveResultSend(aIndex, 0xFF, target, (LPBYTE)&ItemInfo);
			this->CGTradeCancelButtonRecv(aIndex);
		}
		else
		{
...
void GameProtocol::CGUseItemRecv(PMSG_USEITEM* lpMsg, CGameObject &lpObj)
{
	...
	if (CItemObject->IsItem())
	{
		...
		if (CItemObject->m_serial && !gObjCanItemTouch(&gGameObjects[aIndex], 1))
		{
			GCReFillSend(aIndex, lpObj.Life, 0xFD, 1, lpObj.iShield);
			return;
		}
			...
			(CItemObject->m_Type >= ITEMGET(12, 271) && CItemObject->m_Type <= ITEMGET(12, 277))) // Grow Lancer Skills
		{
			...
			{
				int addskill = gObjMagicAdd(&gGameObjects[aIndex], ITEM_GET_TYPE(CItemObject->m_Type), ITEM_GET_INDEX(CItemObject->m_Type), CItemObject->m_Level, skillnumber);
		...
		else if (CItemObject->m_Type == ITEMGET(14, 78)) // CSHOP Arcanum For?a
		{
			...
			g_PeriodItemEx.SetPeriodItemInfo(&gGameObjects[aIndex], CItemObject->m_Type, 0, lpEffect->iEffectValidTime);
			if (!gObjSearchItemMinus(&gGameObjects[aIndex], pos, 1))
			{
				gObjInventoryItemSet(aIndex, pos, -1);
				lpObj.pInventory[pos].Clear();
...
void GameProtocol::GCItemObjectUseSpecialTimeSend(CGameObject &lpObj, unsigned char number, int time)
{
	if (time > 65535)
	{
		time = 65535;
...
void GameProtocol::GCItemObjectDurSend(CGameObject &lpObj, BYTE pos, BYTE dur, unsigned char flag)
{
	PMSG_ITEMDUR pMsg;
...
void GameProtocol::GCItemObjectDurSend2(CGameObject &lpObj, unsigned char pos, unsigned char dur, unsigned char flag)
{
	PMSG_ITEMDUR pMsg;
...
void GameProtocol::CGRequestEnterChaosCastle(PMSG_REQ_MOVECHAOSCASTLE* lpMsg, CGameObject &Obj)
{
		...
		Obj.MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL || g_ConfigRead.server.GetServerType() == SERVER_ARCA)
	{
		...
		IOCP.DataSend(Obj.m_Index, (LPBYTE)&pNotice, pNotice.h.size);
		return;
	}
...
void GameProtocol::GCAnsMapSvrAuth(CGameObject &Obj, int iResult)
{
	...
	IOCP.DataSend(Obj.m_Index, (LPBYTE)&pMsgResult, pMsgResult.h.size);
}
...
void GameProtocol::CGReqCsGateOperate(PMSG_REQ_CSGATEOPERATE * lpMsg, CGameObject &Obj)
{
	...
	if (g_CastleSiege.CheckCsGateAlive(iGateIndex) == TRUE)
	{
		...
		else
		{
			...
			sLog->outBasic("[CastleSiege] CGReqCsGateOperate() OK - [%s][%s], Guild:(%s)(%d), DOOR:(%d)(X:%d,Y:%d)(STATUE:%d)", Obj.AccountID, Obj.Name, Obj.m_PlayerData->GuildName, Obj.m_PlayerData->GuildNumber, iGateIndex, gGameObjects[iGateIndex]->X, gGameObjects[iGateIndex]->Y, gGameObjects[iGateIndex]->m_btCsGateOpen);
		}
	}
	else
	{
...
void GameProtocol::CGReqNpcDbList(PMSG_REQ_NPCDBLIST * lpMsg, CGameObject &Obj)
{
	...
	if (g_CastleSiege.CheckGuildOwnCastle(Obj.m_PlayerData->GuildName) == FALSE || ((Obj.m_PlayerData->GuildStatus != 128) && (Obj.m_PlayerData->GuildStatus != 64)))
	{
		...
		pResult.iCount = 0;
		pResult.btResult = 2;
		IOCP.DataSend(Obj.m_Index, (LPBYTE)&pResult, sizeof(pResult));
	}
	else
	{
...
void GameProtocol::CGReqCsAttkGuildList(PMSG_REQ_CSATTKGUILDLIST * lpMsg, CGameObject &Obj)
{
	...
	IOCP.DataSend(Obj.m_Index, (LPBYTE)cBUFFER, ((lpMsgSend->iCount * sizeof(PMSG_CSATTKGUILDLIST)) + sizeof(PMSG_ANS_CSATTKGUILDLIST)));
}
...
BYTE GameProtocol::EnterITRCheckUser(CGameObject &Obj, int &TroubleUserIndex)
{
	...
	for (int i = 0; i < MAX_USER_IN_PARTY; i++)
	{
		...
		if (useridx > 0)
		{
			...
			if (gGameObjects[useridx]->PartyNumber != Obj.PartyNumber)
			{
				return 13;
			}
