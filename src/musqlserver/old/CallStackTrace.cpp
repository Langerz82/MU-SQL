...
CCallStackTrace g_CallStack("IGC","CrashCallStack","CrashStack");
CCallStackTrace::CCallStackTrace(char * const szLogID,char * const szLogFileName, char * const szLogDirectoryName)
{
	if ( !strcmp(szLogID, ""))
	{
		MessageBox(NULL, "INSERT LOG-ID !!!", "ERROR", MB_OK|MB_TOPMOST);
		return;
	}
...
void CCallStackTrace::Output()
{
	...
	{
		MessageBox(NULL, "NO LOG-ID !!!", "ERROR", MB_OK|MB_TOPMOST);
		return;
	}
...
DWORD CCallStackTrace::ConvertAddress(HANDLE hProcess, DWORD address, LPSTR output_buffer)
{
	...
	if ( SymGetSymFromAddr64(hProcess, address, &displacement, imagehlp_symbol) != FALSE )
	{
		...
		if ( SymGetLineFromAddr64(hProcess, address, &displacement2, &imagehlp_line) != FALSE )
		{
...
int CCallStackTrace::TraceStack(BOOL bWriteInFile)
{
	...
	__try
	{
		...
		for ( int addr_index =0 ;addr_index < address_count ; addr_index++ )
		{
			char symbol[0x504] ={0};
