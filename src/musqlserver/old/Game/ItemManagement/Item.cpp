...
void CItem::Convert(int type, BYTE Option1, BYTE Option2, BYTE Option3, BYTE Attribute2, BYTE SetOption, BYTE ItemEffectEx, BYTE *SocketOption, BYTE SocketBonusOption, BYTE PeriodItemOption, BYTE DbVersion)
{
	...
	//g_Log.AddC(TColor::Yellow, "[K2] Convert %d", _type);
	...
	if (((_type >= ITEMGET(12, 3)) && (_type <= ITEMGET(12, 6))) || ((_type >= ITEMGET(12, 36)) && (_type <= ITEMGET(12, 40))) || (_type == ITEMGET(13, 30)) || (_type == ITEMGET(12, 41)) || (_type == ITEMGET(12, 42)) || (_type == ITEMGET(12, 43)) || (_type == ITEMGET(12, 49)) || (_type == ITEMGET(12, 50)) || ((_type >= ITEMGET(12, 262) && _type <= ITEMGET(12, 265)) || (_type == ITEMGET(12, 269)))) //Third Wings Fix
	{
		Attribute2 = 0;
	}
	...
	std::memset(this->m_SkillResistance, 0, sizeof(this->m_SkillResistance));
	...
	//<Item Index="274" Name="Harsh Strike Scroll" Slot="-1" SkillIndex="0" Width="1" Height="2" Serial="1" Option="0" Drop="0" DropLevel="58" Defense="0" Durability="0" ReqLevel="0" ReqEnergy="0" ReqStrength="75" ReqDexterity="0" ReqCommand="0" Money="100000" DarkWizard="0" DarkKnight="0" FairyElf="0" MagicGladiator="0" DarkLord="0" Summoner="0" RageFighter="0" GrowLancer="1" Type="0" Dump="1" Transaction="1" PersonalStore="1" StoreWarehouse="1" SellToNPC="1" Repair="0" KindA="10" KindB="39" Overlap="0" ModelPath="Data\Item\" ModelFile="SkillScroll.bmd" />
	...
	if (p->RequireLevel != 0)
	{
		...
		else if (_type >= ITEMGET(12, 7) && _type <= ITEMGET(12, 269))
		{
			this->m_RequireLevel = p->RequireLevel;
		}
	...
	if (p->Defense > 0)
	{
		...
		else
		{
			...
			if ((_type >= ITEMGET(12, 36) && _type <= ITEMGET(12, 40)) || _type == ITEMGET(12, 43) || _type == ITEMGET(12, 50) || _type == ITEMGET(12, 268) || _type == ITEMGET(12, 270))
			{
			...
			else if ((_type >= ITEMGET(12, 3) && _type <= ITEMGET(12, 6)) || _type == ITEMGET(13, 30) || _type == ITEMGET(13, 4) || _type == ITEMGET(12, 269))
			{
	...
	std::memset(this->m_Special, 0, sizeof(this->m_Special));
	std::memset(this->m_SpecialValue, 0, sizeof(this->m_SpecialValue));
	this->m_SpecialNum = 0;
	this->m_Option1 = 0;
	this->m_Option2 = 0;
	this->m_Option3 = 0;
	...
	if (this->m_Type >= ITEMGET(12, 200) && this->m_Type <= ITEMGET(12, 220) || this->m_Type >= ITEMGET(12, 306) && this->m_Type <= ITEMGET(12, 308))
	{
	...
	if (SOptionStatValue != 0)
	{
		if (p->SetAttr != 0)
		{
			...
			if (p->SetAttr == 6)
			{
				this->m_SetAddStat = SOptionStatValue * 7.5;
			}
	...
	{
		std::memset(this->m_Resistance, 0, sizeof(this->m_Resistance));
	}
	...
	{
		this->m_Option1 = 0;
		this->m_Special[SPECIAL_OPTION1] = 0;
	...
	else
	{
		...
		this->m_Special[SPECIAL_OPTION1] = ((p->SkillType == SKILL_NONE) ? this->m_Special[SPECIAL_OPTION1] : ((p->SkillType == AT_SKILL_LONGSPEAR) ? SKILL_NONE : p->SkillType));
		this->m_SpecialValue[SPECIAL_OPTION1] = this->m_SpecialValue[SPECIAL_OPTION1];
		this->m_SkillChange = ((p->SkillType == AT_SKILL_LONGSPEAR) ? 1 : 0);
	}

	if (gItemOption.GetItemOption(SPECIAL_OPTION2, this) == 0)
	{
		this->m_Option2 = 0;
		this->m_Special[SPECIAL_OPTION2] = 0;
		this->m_SpecialValue[SPECIAL_OPTION2] = 0;
	}
	else
	{
		this->m_Option2 = ((this->m_Option2 == 0) ? 1 : this->m_Option2);
		this->m_Special[SPECIAL_OPTION2] = this->m_Special[SPECIAL_OPTION2];
	...
	else
	{
		...
		case ITEMGET(12, 4): // Wings of Soul
			this->m_RequireStrength += (this->m_Option3 * 4);
			break;
		case ITEMGET(12, 5): // Wings of Devil
			this->m_RequireStrength += (this->m_Option3 * 4);
			break;
		...
		case ITEMGET(12, 42): // Wings of Despair
			this->m_RequireStrength += (this->m_Option3 * 4);
			break;
		case ITEMGET(12, 49): // Cloak of Fighter
			this->m_RequireStrength += (this->m_Option3 * 4);
			break;
		case ITEMGET(13, 30): // Cloak of Lord
			this->m_RequireStrength += (this->m_Option3 * 4);
			break;
		default:
			this->m_RequireStrength += ((this->m_Type < ITEMGET(12, 0)) ? (this->m_Option3 * 4) : 0);
			break;
		}
	}
	...
	{
		this->m_Special[SPECIAL_EXCELLENT1] = 0;
		this->m_SpecialValue[SPECIAL_EXCELLENT1] = 0;
	}
	else
	{
		this->m_Special[SPECIAL_EXCELLENT1] = this->m_Special[SPECIAL_EXCELLENT1];
	...
	{
		this->m_Special[SPECIAL_EXCELLENT2] = 0;
		this->m_SpecialValue[SPECIAL_EXCELLENT2] = 0;
	}
	else
	{
		this->m_Special[SPECIAL_EXCELLENT2] = this->m_Special[SPECIAL_EXCELLENT2];
	...
	{
		this->m_Special[SPECIAL_EXCELLENT3] = 0;
		this->m_SpecialValue[SPECIAL_EXCELLENT3] = 0;
	}
	else
	{
		this->m_Special[SPECIAL_EXCELLENT3] = this->m_Special[SPECIAL_EXCELLENT3];
	...
	{
		this->m_Special[SPECIAL_EXCELLENT4] = 0;
		this->m_SpecialValue[SPECIAL_EXCELLENT4] = 0;
	}
	else
	{
		this->m_Special[SPECIAL_EXCELLENT4] = this->m_Special[SPECIAL_EXCELLENT4];
	...
	{
		this->m_Special[SPECIAL_EXCELLENT5] = 0;
		this->m_SpecialValue[SPECIAL_EXCELLENT5] = 0;
	}
	else
	{
		this->m_Special[SPECIAL_EXCELLENT5] = this->m_Special[SPECIAL_EXCELLENT5];
	...
	{
		this->m_Special[SPECIAL_EXCELLENT6] = 0;
		this->m_SpecialValue[SPECIAL_EXCELLENT6] = 0;
	}
	else
	{
		this->m_Special[SPECIAL_EXCELLENT6] = this->m_Special[SPECIAL_EXCELLENT6];
	...
	{
		this->m_Special[SPECIAL_COMMON1] = 0;
		this->m_SpecialValue[SPECIAL_COMMON1] = 0;
	}
	else
	{
		this->m_Special[SPECIAL_COMMON1] = this->m_Special[SPECIAL_COMMON1];
	...
	{
		this->m_Special[SPECIAL_COMMON2] = 0;
		this->m_SpecialValue[SPECIAL_COMMON2] = 0;
	}
	else
	{
		this->m_Special[SPECIAL_COMMON2] = this->m_Special[SPECIAL_COMMON2];
	...
	{
		this->m_Special[SPECIAL_COMMON3] = 0;
		this->m_SpecialValue[SPECIAL_COMMON3] = 0;
	}
	else
	{
		this->m_Special[SPECIAL_COMMON3] = this->m_Special[SPECIAL_COMMON3];
	...
	{
		this->m_Special[SPECIAL_COMMON4] = 0;
		this->m_SpecialValue[SPECIAL_COMMON4] = 0;
	}
	else
	{
		this->m_Special[SPECIAL_COMMON4] = this->m_Special[SPECIAL_COMMON4];
	...
	{
		this->m_Special[SPECIAL_COMMON5] = 0;
		this->m_SpecialValue[SPECIAL_COMMON5] = 0;
	}
	else
	{
		this->m_Special[SPECIAL_COMMON5] = this->m_Special[SPECIAL_COMMON5];
...
BOOL CItem::IsExtItem()
{
	...
	if (gSocketItemType.CheckSocketItemType(this->m_Type) == FALSE &&
		g_PentagramSystem.IsPentagramItem(this->m_Type) == FALSE &&
		g_PentagramSystem.IsPentagramJewel(this->m_Type) == FALSE)
	{
		for (int i = 0; i < 5; i++)
		{
			if (this->m_SocketOption[i] != 0xFF)
...
int CItem::DurabilityDown(int dur, int aIndex)
{
	...
	/*if (gGameObjects[aIndex].m_PlayerData->m_MPSkillOpt.iMpsDurabilityReduction2 > 0.0)
	{
		iBaseDurSmall += gGameObjects[aIndex].m_PlayerData->m_MPSkillOpt.iMpsDurabilityReduction2;
	}*/
...
int CItem::DurabilityDown2(int dur, int aIndex)
{
	...
	/*if (gGameObjects[aIndex].m_PlayerData->m_MPSkillOpt.iMpsDurabilityReduction1 > 0.0)
	{
		iBaseDurSmall += gGameObjects[aIndex].m_PlayerData->m_MPSkillOpt.iMpsDurabilityReduction1;
	}*/
...
int CItem::ArmorDurabilityDown(int damagemin, int aIndex)
{
	...
	else if (this->m_RequireClass[7] != 0)	// GL
	{
		this->PlusSpecial(&plusdef, 83);
		DecreaseDur = damagemin / (def * 6 + plusdef);
	}
...
void ItemByteConvert(unsigned char* buf, int type, BYTE Option1, BYTE Option2, BYTE Option3, BYTE level, BYTE dur, BYTE Noption, BYTE SetOption, BYTE JewelOfHarmonyOption, BYTE ItemEffectEx, BYTE *SocketOption, BYTE SocketBonusOption, BYTE PeriodItemOption)
{
	...
	if (pItemAttribute->ItemKindA == 12 && (pItemAttribute->ItemKindB >= 63 && pItemAttribute->ItemKindB <= 66))
	{
#if(TEST_FIX_MUUN == 0)
		buf[n] = 0x12;
		buf[n - 1] = 0x01;
...
int ItemGetDurability(int index, int itemLevel, int ExcellentItem, int SetItem)
{
	...
	if ((index < ITEMGET(12, 3) || index > ITEMGET(12, 6)) &&
		...
		index != ITEMGET(12, 266) &&
		index != ITEMGET(12, 267) &&
		index != ITEMGET(12, 268) &&
		index != ITEMGET(12, 269) &&
		index != ITEMGET(12, 270))
	{
		if (SetItem != 0)
			dur += 20;
...
BOOL OpenItemScript(char* FileName)
{
	...
	std::memset(ItemAttribute, 0, sizeof(ItemAttribute));
	...
	for (pugi::xml_node section = mainXML.child("Section"); section; section = section.next_sibling())
	{
		...
		for (pugi::xml_node item = section.child("Item"); item; item = item.next_sibling())
		{
			...
			if (ItemType == 12)
			{
				if (ItemIndex >= 200 && ItemIndex <= 220 || ItemIndex >= 306 && ItemIndex <= 308)
				{
...
#if(TEST_FIX_MUUN == 1)
			if (ItemType <= 11 || ItemType == 13 || ItemType == 16)
#else
...
int CItem::GetDetailItemType()
{
	...
	{
		if (this->m_TwoHand == TRUE && p->ItemSlot == 0)
		{
...
{
	return GetItemKindA(item_num) == ITEM_KIND_A_BLOODANGEL_SET_ITEM;
}
