...
int QuestExpLuaBind::IsUsingExtendedWarehouse(lua_State *L)
{
	...
	int iExtendedWarehouseCount = getGameObject(iObjIndex)->m_PlayerData->m_WarehouseExpansion;
	lua_settop(L, -2);
...
int QuestExpLuaBind::GetNpcIndex(lua_State *L)
{
	...
	if (getGameObject(iObjIndex)->TargetNpcNumber == -1)
	{
		return 0;
	}

	int iNpcIndex = getGameObject(getGameObject(iObjIndex)->TargetNpcNumber)->Class;
	lua_settop(L, -2);
...
{
	int iObjIndex = luaL_checkinteger(L, 2);
	int iCharType = getGameObject(iObjIndex)->Class;
	lua_settop(L, -2);
...
int QuestExpLuaBind::SubQuestZen(lua_State *L)
{
	...
	if (getGameObject(iObjIndex)->m_PlayerData->Money >= iSubMoney)
	{
		getGameObject(iObjIndex)->m_PlayerData->Money -= iSubMoney;
	}
