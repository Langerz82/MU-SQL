...
bool CItemObjectDrop::DropItem(CGameObject &User, CGameObject lpMonster)
{
	...
	for (int i = 0; i < iItemCount; i++)
	{
		...
		ItemCreate(lpMonster->m_Index, lpMonster->MapNumber, lpMonster->X, lpMonster->Y, ItemNumber, btItemLevel, 0, btSkill, btLuck, btOption, lpUser->m_Index, btExc, btSetItem, dwDuration, btSocketOption, btMainAttribute);
	}
