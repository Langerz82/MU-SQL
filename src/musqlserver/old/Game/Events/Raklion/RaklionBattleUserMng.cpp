...
void CRaklionBattleUserMng::CheckUserState()
{
	for(int iCount=0; iCount<m_BattleUser.size(); iCount++)
	{
		if( !gObjIsConnected(m_BattleUser[iCount]))
		{
			...
			sLog->outBasic("[ RAKLION ][ BattleUser ] Delete User - Disconnect [%s][%s]", getGameObject(m_BattleUser[iCount]]->AccountID, getGameObject(m_BattleUser[iCount))->Name);
		}

		if( getGameObject(m_BattleUser[iCount])->MapNumber != MAP_INDEX_HATCHERY &&
			 getGameObject(m_BattleUser[iCount])->m_State == 2 &&
			 getGameObject(m_BattleUser[iCount])->Live == 1 &&
			 getGameObject(m_BattleUser[iCount])->m_SkillInfo.LordSummonMapNumber != MAP_INDEX_HATCHERY )
		{
			DeleteUserData(m_BattleUser[iCount]);
			sLog->outBasic("[ RAKLION ][ BattleUser ] Delete User - Map Move [%s][%s]", getGameObject(m_BattleUser[iCount]]->AccountID, getGameObject(m_BattleUser[iCount))->Name);
		}
	}
}
...
BOOL CRaklionBattleUserMng::MoveAllUser(int iGateNumber)
{
	...
	for(int iCount=0; iCount<m_BattleUser.size(); iCount++)
	{
		...
		{
			sLog->outBasic("[ RAKLION ][ BattleUser ] [%s][%s] MoveGate(%d)", getGameObject(m_BattleUser[iCount]]->AccountID, getGameObject(m_BattleUser[iCount))->Name, iGateNumber);
		}
		else
		{
			DeleteUserData(m_BattleUser[iCount]);
			sLog->outBasic("[ RAKLION ][ BattleUser ] [%s][%s] MoveGate Fail (%d)", getGameObject(m_BattleUser[iCount]]->AccountID, getGameObject(m_BattleUser[iCount))->Name, iGateNumber);
