	...
	if (max_count && titem->m_Type == sitem->m_Type && titem->m_Level == sitem->m_Level && titem->m_Durability < max_count)
	{
		...
		if (titem->m_Type == ITEMGET(14, 215) && titem->m_Durability >= max_count)
		{
			...
			if (titem->m_Durability == 0.0)
			{
				...
				GSProtocol.GCEventInventoryItemDeleteSend(lpObj, target, TRUE);
				// TODO - Send to player
		...
		if (titem->m_Type == ITEMGET(14, 278) && titem->m_Durability >= max_count)
		{
			...
			if (titem->m_Durability == 0.0)
			{
				gObjEventInventoryItemSet(lpObj->m_Index, target, -1);
				lpObj->pEventInventory[target].Clear();
				GSProtocol.GCEventInventoryItemDeleteSend(lpObj, target, TRUE); // TODO Send Player.
		...
		if (sitem->m_Durability <= 0.0)
		{
			...
			sitem->Clear();
			GSProtocol.GCEventInventoryItemDeleteSend(lpObj->m_Index, source, FALSE); // TODO - Send Player.
			*durSsend = TRUE;
			return 21;
		}
...
BYTE gObjEventInventoryTradeMove(LPGameObject &lpObj, BYTE source, BYTE target)
{
	...
	if (*(BYTE*)(lpObj->TradeMap + h * 8 + w) == 255)
	{
		...
		GSProtocol.GCTradeOtherAdd(lpObj->TargetNumber, blank, itembuf); // TODO Send Player.
		return true;
	}
...
BYTE gObjTradeEventInventoryMove(LPGameObject &lpObj, BYTE source, BYTE target)
{
	...
	GSProtocol.GCTradeOtherDel(lpObj, source); // TODO Send Player
...
BOOL CheckEventInventoryEmptySpace(LPGameObject &lpObj, int iItemHeight, int iItemWidth)
{
	int h = 0;
	int w = 0;
	BYTE blank = 0;
...
BOOL gObjEventInventorySearchSerialNum(LPGameObject &lpObj, UINT64 serial)
{
	UINT64 s_num;
	int count = 0;
	...
	for (int n = 0; n < EVENT_INVENTORY_SIZE; n++)
	{
		...
		if (s_num != 0 && s_num == serial && s_num != (UINT64)-1)
		{
			...
			GSProtocol.GCServerMsgStringSend(Lang.GetText(0, 15), lpObj->m_Index, 1); // TODO Send to game server.
			gObjUserKill(lpObj);
		}
	}
