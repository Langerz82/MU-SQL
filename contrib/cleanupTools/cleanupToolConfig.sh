...
#               Generic options:
############################################################
# Define the path to <MuMySQLServer> folder relatively from where you want to call the scripts.
# The cleanupHistory.sh script must be called from within the repository that is to be cleaned!
#
BASEPATH="."
#
# Path to Astyle from the <MuMySQLServer> directory
...
DO="ASTYLE"
#
#   DO_ON with possible choices:    MANGOS_SRC      to invoke the above cleanup routine on <MuMySQLServer>/src directory and subdirectories
#                                   MANGOS_CONTRIB  to invoke the above cleanup routine on <MuMySQLServer>/contrib directory and subdirectories
#                                   MANGOS_WHOLE    to invoke the above cleanup routine on <MuMySQLServer> directory and subdirectories
#                                   SD2             to invoke the above cleanup routine on <MuMySQLServer>/src/bindings/ScriptDev2 directory and subdirectories
DO_ON="MANGOS_SRC"
